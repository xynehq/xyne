import { createFileRoute, Link, useNavigate } from "@tanstack/react-router"
import { FileJson, Globe, Plus } from "lucide-react"
import { useEffect, useMemo, useState } from "react"
import { Topbar } from "@/components/Topbar"
import { SearchField } from "@/components/file-browser"
import { type Agent, type AgentListFilter, listAgents } from "@/lib/api"

type ExtractorsSearch = {
  tab?: AgentListFilter
  q?: string
}

export const Route = createFileRoute("/_authenticated/extractors/")({
  validateSearch: (raw: Record<string, unknown>): ExtractorsSearch => {
    const out: ExtractorsSearch = {}
    if (
      raw["tab"] === "all" ||
      raw["tab"] === "madeByMe" ||
      raw["tab"] === "sharedToMe"
    ) {
      out.tab = raw["tab"]
    }
    if (typeof raw["q"] === "string" && raw["q"] !== "") {
      out.q = raw["q"]
    }
    return out
  },
  component: ExtractorsIndexRoute,
})

const TABS: ReadonlyArray<{ id: AgentListFilter; label: string }> = [
  { id: "all", label: "All" },
  { id: "madeByMe", label: "Mine" },
  { id: "sharedToMe", label: "Shared" },
]

function ExtractorsIndexRoute(): JSX.Element {
  const search = Route.useSearch()
  const navigate = useNavigate({ from: Route.fullPath })
  const tab: AgentListFilter = search.tab ?? "all"
  const query = search.q ?? ""

  const [extractors, setExtractors] = useState<Agent[] | null>(null)
  const [error, setError] = useState<string | null>(null)

  useEffect((): (() => void) => {
    let cancelled = false
    setExtractors(null)
    setError(null)
    listAgents({ limit: 200, offset: 0, filter: tab, isExtractor: true })
      .then(({ agents: list }): void => {
        if (!cancelled) setExtractors(list)
      })
      .catch((err: Error): void => {
        if (!cancelled) setError(err.message)
      })
    return (): void => {
      cancelled = true
    }
  }, [tab])

  const filtered = useMemo<Agent[]>(() => {
    if (!extractors) return []
    const needle = query.trim().toLowerCase()
    const base = needle
      ? extractors.filter(
          (a) =>
            a.name.toLowerCase().includes(needle) ||
            a.description.toLowerCase().includes(needle),
        )
      : extractors
    return base.slice().sort((a, b) => a.name.localeCompare(b.name))
  }, [extractors, query])

  const setQuery = (next: string): void => {
    void navigate({
      replace: true,
      search: (prev: ExtractorsSearch): ExtractorsSearch => {
        const out: ExtractorsSearch = {}
        if (prev.tab !== undefined) out.tab = prev.tab
        if (next !== "") out.q = next
        return out
      },
    })
  }

  const setTab = (next: AgentListFilter): void => {
    void navigate({
      replace: true,
      search: (prev: ExtractorsSearch): ExtractorsSearch => {
        const out: ExtractorsSearch = {}
        if (next !== "all") out.tab = next
        if (prev.q !== undefined) out.q = prev.q
        return out
      },
    })
  }

  const count = filtered.length
  const showingSearch = query.trim().length > 0

  return (
    <div className="flex h-full flex-col">
      <Topbar title="Extractors" />

      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border bg-background/70 px-5 py-2.5 backdrop-blur-md">
        <div className="flex min-w-0 flex-1 items-center gap-2">
          <span className="text-[13px] font-medium text-foreground">
            Extractors
          </span>
        </div>

        <div className="flex flex-wrap items-center gap-2">
          <TabBar value={tab} onChange={setTab} />
          <Link
            to="/extractors/new"
            className="inline-flex h-7 items-center gap-1 rounded-md border border-border bg-surface-elevated px-2 text-[12px] text-foreground transition hover:bg-secondary"
          >
            <Plus className="h-3.5 w-3.5" strokeWidth={1.75} />
            New extractor
          </Link>
          <SearchField
            value={query}
            onChange={setQuery}
            className="w-56"
            ariaLabel="Search extractors"
            placeholder="Search extractors"
          />
        </div>
      </div>

      <main className="flex-1 overflow-auto px-5 py-5">
        <div className="mx-auto w-full max-w-7xl">
          <p className="mb-3 text-[12px] text-muted-foreground">
            {extractors === null
              ? "Loading…"
              : count === 0
                ? showingSearch
                  ? "No matches"
                  : tab === "madeByMe"
                    ? "You haven't created any extractors"
                    : tab === "sharedToMe"
                      ? "Nothing has been shared with you"
                      : "No extractors in this workspace"
                : `${String(count)} extractor${count === 1 ? "" : "s"}`}
          </p>

          {error ? (
            <ErrorPane message={error} />
          ) : extractors === null ? (
            <SkeletonGrid />
          ) : count === 0 ? (
            <EmptyPane
              tab={tab}
              searching={showingSearch}
              query={query}
            />
          ) : (
            <ul
              role="list"
              className="grid animate-fade-up grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4"
            >
              {filtered.map((extractor) => (
                <li key={extractor.externalId}>
                  <ExtractorCard extractor={extractor} />
                </li>
              ))}
            </ul>
          )}
        </div>
      </main>
    </div>
  )
}

function TabBar({
  value,
  onChange,
}: {
  value: AgentListFilter
  onChange: (next: AgentListFilter) => void
}): JSX.Element {
  return (
    <div
      role="group"
      aria-label="Filter"
      className="inline-flex items-center rounded-full border border-border bg-surface-elevated p-0.5"
    >
      {TABS.map((t) => {
        const active = value === t.id
        return (
          <button
            key={t.id}
            type="button"
            onClick={(): void => onChange(t.id)}
            aria-pressed={active}
            className={
              "inline-flex h-7 items-center rounded-full px-2.5 text-[12px] transition " +
              (active
                ? "bg-secondary text-foreground"
                : "text-muted-foreground hover:text-foreground")
            }
          >
            {t.label}
          </button>
        )
      })}
    </div>
  )
}

function ExtractorCard({ extractor }: { extractor: Agent }): JSX.Element {
  const fieldCount = countSchemaFields(extractor.responseSchema)
  return (
    <Link
      to="/extractors/$extractorId"
      params={{ extractorId: extractor.externalId }}
      className="group flex h-full w-full flex-col items-start gap-3 rounded-2xl border border-border bg-surface-elevated p-4 text-left transition hover:border-ring/40 hover:bg-secondary/60 active:scale-[0.99]"
      title={extractor.name}
    >
      <span
        aria-hidden
        className="grid h-10 w-10 flex-shrink-0 place-items-center rounded-xl bg-surface-muted text-foreground"
      >
        <FileJson className="h-5 w-5" strokeWidth={1.5} />
      </span>
      <span className="flex w-full min-w-0 flex-col gap-0.5">
        <span className="truncate text-[13.5px] font-medium text-foreground">
          {extractor.name}
        </span>
        <span className="line-clamp-2 min-h-[2rem] text-[11.5px] leading-snug text-muted-foreground">
          {extractor.description || (
            <span className="italic text-muted-foreground/70">
              No description.
            </span>
          )}
        </span>
      </span>
      <div className="mt-auto flex flex-wrap items-center gap-1.5">
        <Pill>
          {fieldCount === 0
            ? "No fields"
            : `${String(fieldCount)} field${fieldCount === 1 ? "" : "s"}`}
        </Pill>
        {extractor.isPublic ? (
          <Pill icon={<Globe className="h-3 w-3" aria-hidden strokeWidth={1.75} />}>
            Public
          </Pill>
        ) : null}
      </div>
    </Link>
  )
}

function countSchemaFields(
  schema: Record<string, unknown> | null | undefined,
): number {
  if (!schema || typeof schema !== "object") return 0
  const props = schema["properties"]
  if (!props || typeof props !== "object") return 0
  return Object.keys(props as Record<string, unknown>).length
}

function Pill({
  children,
  icon,
}: {
  children: React.ReactNode
  icon?: JSX.Element
}): JSX.Element {
  return (
    <span className="inline-flex items-center gap-1 rounded-full bg-secondary px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-[0.06em] text-muted-foreground">
      {icon}
      {children}
    </span>
  )
}

function SkeletonGrid(): JSX.Element {
  return (
    <ul
      role="list"
      className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4"
    >
      {Array.from({ length: 8 }).map((_, i) => (
        <li
          key={i}
          className="h-[148px] animate-breathe rounded-2xl border border-border bg-surface-elevated"
          aria-hidden
        />
      ))}
    </ul>
  )
}

function EmptyPane({
  tab,
  searching,
  query,
}: {
  tab: AgentListFilter
  searching: boolean
  query: string
}): JSX.Element {
  const headline = searching
    ? "No matches"
    : tab === "madeByMe"
      ? "You haven't created any extractors"
      : tab === "sharedToMe"
        ? "Nothing has been shared with you"
        : "No extractors in this workspace"
  const detail = searching
    ? `We couldn't find anything matching "${query}". Try a broader term.`
    : "Extractors turn unstructured chats into structured JSON — define a schema and the model validates its output before returning."
  return (
    <div className="mx-auto flex max-w-md flex-col items-center justify-center gap-3 py-24 text-center">
      <span className="grid h-12 w-12 place-items-center rounded-2xl bg-surface-muted text-muted-foreground">
        <FileJson className="h-5 w-5" aria-hidden strokeWidth={1.5} />
      </span>
      <p className="text-[14px] font-medium text-foreground">{headline}</p>
      <p className="max-w-xs text-[12.5px] text-muted-foreground">{detail}</p>
      {!searching ? (
        <Link
          to="/extractors/new"
          className="mt-1 inline-flex h-7 items-center gap-1 rounded-md border border-border bg-surface-elevated px-2 text-[12px] text-foreground transition hover:bg-secondary"
        >
          <Plus className="h-3.5 w-3.5" strokeWidth={1.75} />
          New extractor
        </Link>
      ) : null}
    </div>
  )
}

function ErrorPane({ message }: { message: string }): JSX.Element {
  return (
    <div className="mx-auto max-w-md rounded-2xl border border-destructive/30 bg-destructive/5 px-4 py-3 text-[13px] text-destructive">
      Couldn't load extractors — {message}
    </div>
  )
}
