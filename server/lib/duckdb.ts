...
export const querySheetChunks = async (
  ...
  Logger.debug(
    `Writing ${cleanedSheetChunks.length} chunks to temporary file: ${tempFilePath}`,
  )
  ...
  await connection.run(
    `PRAGMA threads=${Math.max(1, Math.floor(cpus().length / 2))}`,
  )
  ...
  try {
    ...
    const sampleRowsHeader = schemaResult
      .map((c: any) => c.column_name)
      .join("\t")
    const sampleRowsBody = sampleRowsRes
      .map((r: any) =>
        schemaResult.map((c: any) => String(r[c.column_name] ?? "")).join("\t"),
      )
      .join("\n")
    ...
    if (validationResult.warnings && validationResult.warnings.length > 0) {
      Logger.warn(
        `SQL validation warnings: ${validationResult.warnings.join(", ")}`,
      )
    }
    ...
    Logger.debug(
      `Query executed successfully, returned ${result.length} rows in ${elapsedMs}ms`,
    )
    ...
    const resultPackage: DuckDBResult = {
      ...
      schema_fragment: {
        ...
        columns: schemaResult.reduce(
          (acc: Record<string, string>, col: any) => {
