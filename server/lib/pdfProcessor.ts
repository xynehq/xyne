...
} from "@/lib/appSyncPdfLibSemaphore"
...
export class PdfProcessor {
  ...
  static async loadDocument(
    ...
    try {
      ...
      if (isAppSyncPdfLibSemaphoreTimeoutError(error)) {
        Logger.error(
          {
            fileId: context.fileId,
            fileName: context.fileName,
          ...
          "🔴 PDF-lib load_document failed while waiting for permit",
        )
        throw error
      }
    ...
    try {
      ...
      if (isAppSyncPdfLibSemaphoreTimeoutError(error)) {
        Logger.error(
          ...
          "🔴 qpdf page count failed while waiting for permit",
        )
        throw error
      }
