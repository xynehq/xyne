...
import {
  acquireDoclingAsyncSubmitPermit,
  releaseDoclingAsyncSubmitPermit,
  type DoclingAsyncSubmitPermit,
} from "@/lib/doclingAsyncSubmitPermits"
...
const releaseSubmitPermitSafely = async (
  permit: DoclingAsyncSubmitPermit,
  context: Record<string, unknown>,
) => {
  try {
    await releaseDoclingAsyncSubmitPermit(permit.jobId)
  } catch (error) {
    Logger.error(
      {
        ...context,
      ...
      "Failed to release async Docling submit permit",
    )
  }
}
...
export const submitDoclingAsyncJob = async (
  ...
  for (let attempt = 1; attempt <= SUBMIT_RETRIES; attempt++) {
    let permit: DoclingAsyncSubmitPermit | null = null
    ...
    try {
      if (
        config.doclingAsyncSubmitPermitsEnabled &&
        !input.skipInternalPermit
      ) {
        permit = await acquireDoclingAsyncSubmitPermit(input)
      }
      ...
      if (permit) {
        await releaseSubmitPermitSafely(permit, {
          fileId: input.fileId,
