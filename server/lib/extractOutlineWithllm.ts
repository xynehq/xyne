...
export const extractOutlineWithllm = async (textContext: string): Promise<string | undefined> => {
...
--- DOCUMENT END ---`;
  ...
  const liteLlmBaseUrl = process.env.LITELLM_BASE_URL || process.env.OPENAI_API_BASE;
  const liteLlmApiKey = process.env.LITELLM_API_KEY || process.env.OPENAI_API_KEY;
  const liteLlmModel = process.env.LITELLM_FAST_MODEL || process.env.LITELLM_BEST_MODEL || "gpt-4o-mini";
  
  if (liteLlmBaseUrl && liteLlmApiKey) {
    try {
      Logger.debug("Using raw LiteLLM/OpenAI compatible endpoint for outline extraction")
      const endpoint = liteLlmBaseUrl.endsWith("/v1") ? liteLlmBaseUrl + "/chat/completions" : liteLlmBaseUrl + "/v1/chat/completions";
      ...
      if (!text && data?.output && Array.isArray(data.output)) {
         ...
         if (msg && msg.content && Array.isArray(msg.content)) {
            const txtMsg = msg.content.find((c: any) => c.type === "output_text" || c.text);
            if (txtMsg) text = txtMsg.text;
         }
      }

      if (text && !text.includes("NO_OUTLINE") && text.length > 10) return text.trim();
      return undefined;
    } catch (err) {
  ...
  const projectId = process.env.VERTEX_PROJECT_ID || process.env.GOOGLE_CLOUD_PROJECT || process.env.GCLOUD_PROJECT || process.env.GCP_PROJECT_ID
  if (projectId) {
    try {
      Logger.debug("Using Vertex AI for outline extraction")
      const location = process.env.VERTEX_REGION || "us-central1"
      const modelId = process.env.VERTEX_AI_MODEL_PDF_PROCESSING || "gemini-2.5-flash"
  ...
  Logger.debug("No LLM configured or all extractions failed for Document Outline fallback")
  return undefined
}
