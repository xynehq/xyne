...
export class SQLValidator {
  ...
  private collectFunctionNames(ast: any, out: Set<string>): void {
    ...
    for (const key of [
      ...
      if (Array.isArray(arr))
        arr.forEach((node: any) => this.collectFunctionNames(node, out))
    }
  }
  ...
  private validateTableAccess(sql: string): SQLValidationResult {
    try {
      ...
      if (!allowedViewName) {
        return {
          isValid: false,
          error: "allowedViewName or allowedTableNames is required",
        }
      }
  ...
  private hasWindowFunctionInExpression(expr: any): boolean {
    ...
    if (expr.right && this.hasWindowFunctionInExpression(expr.right))
      return true
    if (expr.operand && this.hasWindowFunctionInExpression(expr.operand))
      return true
  ...
  private collectWarnings(ast: any): string[] {
    ...
    if (this.hasWindowFunctions(ast)) {
      warnings.push(
        "Query contains window functions which may impact performance",
      )
    }
...
export function validatePostgresQuery(
  ...
  allowedTableNames: string[],
  options?: Partial<
    Pick<SQLValidationOptions, "allowSubqueries" | "allowCTEs">
  >,
): SQLValidationResult {
  const validator = new SQLValidator({
    allowedTableNames,
