...
async function loadTestImageBuffer(): Promise<Buffer> {
  ...
  try {
    ...
    console.log(`✅ Real image loaded successfully. Size: ${imageBuffer.length} bytes`);
    return imageBuffer;
  } catch (error) {
    console.error(`❌ Failed to load image from ${imagePath}:`, (error as Error).message);
    console.log("🔄 Falling back to test image buffer...");
    // Fallback to a test image if the real one can't be loaded
    const whitePixelPng = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8/5+hHgAHggJ/PchI7wAAAABJRU5ErkJggg==";
          ...
          parsing_res_list: [
            {
              ...
              block_content: "Building blocks poised to bring 10X impact for our travel partners, across the ecosystem",
              block_bbox: [393, 136, 2433, 342]
            },
            {
              block_label: "image",
              block_content: "Full Suite FinOps Tool Orchestration & FRM virtual cards maker-checker reporting & insights smart & dynamic routing custom FRM engines AI based recon supplier payments notifications 300+ global PSPs & Payment methods flag anomalies Expense Management Detect anomalies Split settlements Smart Retries Superior Checkout Experience Reliable Global Coverage High Efficiency Low risk & Cost 1-click checkout customizable UiUx multiple payment networks 15+ currencies seamless 3DS experience Offers & loyalty engine single API Integration work with global banks Tokenization & card vault refunds & chargebacks GDS VISA local payment methods multi language support Networks HSBC Boost Conversions OTA/TMCs Global Coverage Acquirers Customer(CTA/BTA) IATA BSP",
      ...
      },
      {
        prunedResult: {
          ...
          parsing_res_list: [
            ...
            {
              block_label: "text",
              block_content: "If a transaction fails because of a technical reason, we retry the transaction with a different PSP.",
              block_bbox: [241, 704, 989, 845]
            },
            {
              block_label: "text",
              block_content: "The decision whether to retry or not is based on error reason.",
            ...
            {
              block_label: "text",
              block_content: "In case a transaction fails with a definitive error message, we redirect the user to a Payment Retry screen and nudge the user to use a different Payment Instrument",
              block_bbox: [240, 1130, 1051, 1329]
            },
            {
              block_label: "table",
              block_content: "<html><body><table><tbody><tr><td>Merchant ID</td><td>Success GMV</td><td>Total Success Transactions</td><td>Total Retried GMV</td><td>Total Retried Transactions</td><td>Success Rate</td></tr><tr><td>9</td><td>352,793</td><td>798</td><td>539,021</td><td>1,163</td><td>68.62%</td></tr><tr><td></td><td>229,852</td><td>513</td><td>344,956</td><td>745</td><td>68.86%</td></tr></tbody></table></body></html>",
              block_bbox: [1207, 627, 2834, 1333]
            },
            {
              block_label: "vision_footnote",
              block_content: "For the first merchant, we retried 1,163 transactions over a 24 hr window and 798 were successful",
  ...
  try {
    ...
    result1.chunks.forEach((chunk, index) => {
      ...
      if (result1.chunks_map[index]) {
        ...
        console.log(`Block labels: [${result1.chunks_map[index].block_labels.join(", ")}]`);
      }
    });
    ...
    result1.image_chunks.forEach((imageChunk, index) => {
      ...
      if (result1.image_chunks_map[index]) {
        ...
        console.log(`Block labels: [${result1.image_chunks_map[index].block_labels.join(", ")}]`);
      }
    });
    ...
    console.log(`Image chunks metadata count: ${result1.image_chunks_map.length}`);
    ...
    try {
      ...
      const sampleText = "This is a sample document buffer for testing OCR processing capabilities with dummy API responses.";
      const testBuffer = Buffer.from(sampleText, 'utf-8');
      const testFileName = "test-document.pdf";
      const testDocId = "buffer-test-doc-67890";

      console.log(`🔧 Calling chunkByOCRFromBuffer with buffer (${testBuffer.length} bytes)...\n`);

      // Call the buffer-based function
      const result2 = await chunkByOCRFromBuffer(testBuffer, testFileName, testDocId);
      ...
      result2.chunks.forEach((chunk, index) => {
        ...
        if (result2.chunks_map[index]) {
          ...
          console.log(`Block labels: [${result2.chunks_map[index].block_labels.join(", ")}]`);
        }
      });
      ...
      result2.image_chunks.forEach((imageChunk, index) => {
        console.log(`\nImage ${index + 1}:`);
        console.log(`Description: "${imageChunk.substring(0, 100)}${imageChunk.length > 100 ? '...' : ''}"`);
        if (result2.image_chunks_map[index]) {
          console.log(`Page: ${result2.image_chunks_map[index].page_number}`);
          console.log(`Block labels: [${result2.image_chunks_map[index].block_labels.join(", ")}]`);
        }
      });
      ...
      console.log(`Image chunks metadata count: ${result2.image_chunks_map.length}`);
    ...
    console.log("🔍 Check the console logs above for detailed debugging information from the chunkByOCR functions.");
    console.log("📁 Check the downloads/xyne_images_db/ directory for saved images.");
