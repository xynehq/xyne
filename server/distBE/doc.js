 ...
 */
const getImageFromInlineObject = (document, inlineObjectElement) => {
    const inlineObjectId = inlineObjectElement.inlineObjectId;
    if (inlineObjectId) {
        const embeddedObject = document.inlineObjects?.[inlineObjectId]?.inlineObjectProperties
            ?.embeddedObject;
 ...
 */
const extractParagraphContent = (document, paragraph) => {
 ...
 */
const getHeadingPrefix = (paragraph) => {
    const headingMatch = paragraph.paragraphStyle?.namedStyleType?.match(/HEADING_(\d+)/);
 ...
 */
export const extractText = (document, element, nestLevel = 0) => {
    let text = "";
