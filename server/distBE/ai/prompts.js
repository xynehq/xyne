...
export const askQuestionSelfCleanupPrompt = (query, context) => `
  User query: ${query}
  ...
  ${context}
  `;
export const askQuestionUserPrompt = (query, context, userCtx) => `${userCtx ? "Context of the user asking the query: " + userCtx + "\n" : ""}User query: ${query}
...
const userChatSystemPrompt = "You are a knowledgeable assistant that provides accurate and up-to-date answers based on the given context.";
export const userChatSystem = (userCtx) => `${userChatSystemPrompt}\n${userCtx ? "Context of the user you are chatting with: " + userCtx + "\n" : ""}
...
export const generateTitleSystemPrompt = `
  ...
  `;
export const chatWithCitationsSystemPrompt = (userCtx) => `
...
`;
export const analyzeInitialResultsOrRewriteSystemPrompt = (userCtx) => `You are an assistant tasked with evaluating search results from a database of documents, users, and emails, and answering questions based on the provided context.
...
export const analyzeInitialResultsOrRewriteV2SystemPrompt = (userCtx) => `You are an assistant tasked with evaluating search results from a database of documents, users, and emails, and answering questions based on the provided context.
...
export const rewriteQuerySystemPrompt = (hasContext) => `
...
`;
export const optimizedPrompt = (ctx) => `
...
export const generateMarkdownTableSystemPrompt = (userCtx, query) => `
...
User Query: ${query}
`;
export const baselinePrompt = (userContext, retrievedContext) => `You are an AI assistant with access to internal workspace data. You have access to the following types of data:
...
export const baselinePromptJson = (userContext, retrievedContext) => `You are an AI assistant with access to internal workspace data. You have access to the following types of data:
1. Files (documents, spreadsheets, etc.)
2. User profiles
3. Emails
4. Calendar events
...
export const queryRewritePromptJson = (userContext, retrievedContext) => `You are an AI assistant helping to rewrite search queries to find information in a workspace. The original search was unsuccessful in finding a complete answer.
...
export const temporalEventClassifier = (query) => `Determine if this query is specifically asking about tracking down a calendar event or email interaction that either last occurred or will next occur.
...
export const searchQueryPrompt = (userContext) => {
    return `
...
export const searchQueryReasoningPrompt = (userContext) => {
    return `
    <think>
...
export const searchQueryReasoningPromptV2 = (userContext) => {
    return `
    <think>
...
export const meetingPromptJson = (userContext, retrievedContext) => `You are an AI assistant helping find meeting information from both calendar events and emails. You have access to:
...
`;
export const baselineReasoningPromptJson = (userContext, retrievedContext) => `You are an AI assistant with access to internal workspace data. You have access to the following types of data:
1. Files (documents, spreadsheets, etc.)
2. User profiles
3. Emails
4. Calendar events
