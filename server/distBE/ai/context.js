import { eventSchema, fileSchema, mailAttachmentSchema, mailSchema, userSchema, VespaSearchResultsSchema, } from "../search/types.js";
import { getRelativeTime } from "../utils.js";
import pc from "picocolors";
// Utility to capitalize the first letter of a string
const capitalize = (str) => str.charAt(0).toUpperCase() + str.slice(1);
// Function for handling file context
const constructFileContext = (fields, relevance, maxSummaryChunks) => {
    if (!maxSummaryChunks) {
...
const constructUserContext = (fields, relevance) => {
    return `App: ${fields.app}
Entity: ${fields.entity}
...
const constructMailContext = (fields, relevance, maxSummaryChunks) => {
    if (!maxSummaryChunks) {
        maxSummaryChunks = fields.chunks_summary?.length;
    }
    return `App: ${fields.app}
...
const constructMailAttachmentContext = (fields, relevance, maxSummaryChunks) => {
    if (!maxSummaryChunks) {
        maxSummaryChunks = fields.chunks_summary?.length;
    }
    return `App: ${fields.app}
...
const constructEventContext = (fields, relevance) => {
    return `App: ${fields.app}
Entity: ${fields.entity}
...
Attendees: ${fields.attendees && fields.attendees.length
        ? fields.attendees
            .map((attendee) => `${attendee.email} ${attendee.displayName}`)
            .join(", ")
        : "No attendees listed"}
Recurrence: ${fields.recurrence && fields.recurrence.length
...
Cancelled Instances: ${fields.cancelledInstances && fields.cancelledInstances.length
        ? fields.cancelledInstances.join(", ")
        : "No cancelled instances"}
${relevance ? `vespa relevance score: ${relevance}` : ""}`;
};
// Function for handling file context
const constructFileMetadataContext = (fields, relevance) => {
    return `App: ${fields.app}
Entity: ${fields.entity}
Title: ${fields.title ? `Title: ${fields.title}` : ""}
Created: ${getRelativeTime(fields.createdAt)}
...
// TODO: tell if workspace that this is an employee
const constructUserMetadataContext = (fields, relevance) => {
    return `App: ${fields.app}
Entity: ${fields.entity}
Added: ${getRelativeTime(fields.creationTime)}
${fields.name ? `Name: ${fields.name}` : ""}
...
const constructMailMetadataContext = (fields, relevance) => {
    return `App: ${fields.app}
Entity: ${fields.entity}
Sent: ${getRelativeTime(fields.timestamp)}
${fields.subject ? `Subject: ${fields.subject}` : ""}
...
const constructMailAttachmentMetadataContext = (fields, relevance) => {
    return `App: ${fields.app}
Entity: ${fields.entity}
...
const constructFileColoredContext = (fields, relevance) => {
...
const constructUserColoredContext = (fields, relevance) => {
    return `${pc.green("App")}: ${fields.app}
${pc.green("Entity")}: ${fields.entity}
...
const constructMailColoredContext = (fields, relevance) => {
    return `${pc.green("App")}: ${fields.app}
${pc.green("Entity")}: ${fields.entity}
...
export const answerMetadataContextMap = (searchResult) => {
    if (searchResult.fields.sddocname === fileSchema) {
        return constructFileMetadataContext(searchResult.fields, searchResult.relevance);
    }
    else if (searchResult.fields.sddocname === userSchema) {
        return constructUserMetadataContext(searchResult.fields, searchResult.relevance);
    }
    else if (searchResult.fields.sddocname === mailSchema) {
        return constructMailMetadataContext(searchResult.fields, searchResult.relevance);
    }
    else if (searchResult.fields.sddocname === mailAttachmentSchema) {
        return constructMailAttachmentMetadataContext(searchResult.fields, searchResult.relevance);
    }
...
export const answerColoredContextMap = (searchResult) => {
    if (searchResult.fields.sddocname === fileSchema) {
        return constructFileColoredContext(searchResult.fields, searchResult.relevance);
    }
    else if (searchResult.fields.sddocname === userSchema) {
        return constructUserColoredContext(searchResult.fields, searchResult.relevance);
    }
    else if (searchResult.fields.sddocname === mailSchema) {
        return constructMailColoredContext(searchResult.fields, searchResult.relevance);
    }
    else {
        throw new Error(`Invalid search result type: ${searchResult.fields.sddocname}`);
    }
};
export const answerContextMap = (searchResult, maxSummaryChunks) => {
    if (searchResult.fields.sddocname === fileSchema) {
        ...
        return constructMailAttachmentContext(searchResult.fields, searchResult.relevance, maxSummaryChunks);
    }
    else {
        throw new Error(`Invalid search result type: ${searchResult.fields.sddocname}`);
    }
};
export const cleanContext = (text) => {
    return cleanDocs(cleanVespaHighlights(text));
};
export const cleanColoredContext = (text) => {
    return cleanColoredDocs(cleanVespaHighlights(text));
};
const cleanVespaHighlights = (text) => {
    const hiTagPattern = /<\/?hi>/g;
    return text.replace(hiTagPattern, "").trim();
};
const cleanColoredDocs = (text) => {
    const urlPattern = /!\[.*?\]\(https:\/\/lh7-rt\.googleusercontent\.com\/docsz\/[a-zA-Z0-9-_?=&]+\)/g;
...
export const userContext = ({ user, workspace, }) => {
    const now = new Date();
