...
let queriesPath = "", outputPath = "";
if (!args || args.length < expectedArgsLen) {
    throw new Error("path not provided, this script requires --queries path/to/queries.jsonl --output path/to/output/.tsv");
}
const argMap = {};
...
const processedResultsData = [];
let counts = 0;
const user = "junaid.s@xynehq.com";
const evaluate = async (queriesListPath) => {
    const k = 10;
    const queue = new PQueue({ concurrency: 10 });
    const processQuery = async ({ query, query_id, }) => {
        try {
