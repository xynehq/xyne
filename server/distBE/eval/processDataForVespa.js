...
let corpusPath = "", outputPath = "";
if (!args || args.length < expectedArgsLen) {
    throw new Error("path not provided, this script requires --corpus path/to/corpus.jsonl --output path/to/output/.json");
}
const argMap = {};
...
const processVespaDoc = (data) => {
    ...
    return {
        ...
        fields: {
            ...
            chunks: chunks.map((v) => v.chunk.replace(/[\x00-\x1F\x7F]/g, " ").trim()),
            permissions: [user],
        },
    };
};
    ...
    try {
        for await (const line of rl) {
            ...
            try {
                ...
                Logger.error(`Error processing line ${linesRead}: ${error.stack}`);
        ...
        Logger.error(`Processing failed: ${error.message} ${error.stack}`);
        throw error;
    }
};
const writeProcessDocs = async (writeStream, data, isFirstBatch) => {
