...
import { connectors, oauthProviders, selectConnectorSchema } from "./schema.js"
...
export const insertConnector = async (trx, workspaceId, userId, workspaceExternalId, name, type, // Use TypeScript enum for type safety
authType, // Use TypeScript enum for authType
app, // Use TypeScript enum for app
config, credentials, subject, oauthCredentials, status) => {
    ...
    try {
        ...
        Logger.error(error, `Error inserting connection:, ${error} \n ${error.stack}`);
...
export const getConnectors = async (workspaceId, userId) => {
    const res = await db
        .select({
        ...
        .from(connectors)
        .where(and(eq(connectors.workspaceExternalId, workspaceId), eq(connectors.userId, userId)));
    return res;
};
// don't call this
// call the function that ensures the credentials are always refreshed
export const getConnector = async (trx, connectorId) => {
    const res = await db
        .select()
        .from(connectors)
...
const IsTokenExpired = (app, oauthCredentials, bufferInSeconds) => {
    if (IsGoogleApp(app)) {
...
export const getOAuthConnectorWithCredentials = async (trx, connectorId) => {
    const res = await trx
        .select()
        .from(connectors)
        .where(and(eq(connectors.id, connectorId), eq(connectors.authType, AuthType.OAuth)))
        .limit(1);
    if (!res.length) {
    ...
    const oauthRes = selectConnectorSchema.parse(res[0]);
    ...
    if (IsTokenExpired(oauthRes.app, oauthRes.oauthCredentials, 5 * 60)) {
        ...
        if (IsGoogleApp(oauthRes.app)) {
            ...
            const providers = await getOAuthProviderByConnectorId(trx, connectorId);
            if (!providers.length) {
            ...
            const refreshedTokens = await google.refreshAccessToken(tokens.refresh_token);
...
export const getConnectorByExternalId = async (connectorId) => {
    const res = await db
        .select()
        .from(connectors)
...
export const updateConnector = async (trx, connectorId, updateData) => {
