...
import { messages, selectMessageSchema } from "../db/schema.js"
import { MessageRole } from "../types.js"
import { and, asc, eq, lt } from "drizzle-orm";
import { z } from "zod";
export const insertMessage = async (trx, message) => {
    ...
    if (!parsedData.success) {
        throw new Error(`Could not get message after inserting: ${parsedData.error.toString()}`);
    }
    return parsedData.data;
};
export const getChatMessages = async (trx, chatId) => {
...
export const getChatMessagesBefore = async (trx, chatId, createdAt) => {
    const messagesArr = await trx
        .select()
        .from(messages)
...
export const getMessageByExternalId = async (trx, messageId) => {
    const messageArr = await trx
        .select()
        .from(messages)
...
export const updateMessage = async (trx, messageId, updatedFields) => {
    await trx
        .update(messages)
        .set(updatedFields)
