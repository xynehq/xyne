...
import { Subsystem, SyncCron } from "../../types.js"
...
import { getAppSyncJobs, updateSyncJob } from "../../db/syncJob.js"
import { getUserById } from "../../db/user.js"
import { insertSyncHistory } from "../../db/syncHistory.js"
import { getErrorMessage, retryWithBackoff } from "../../utils.js"
import {
  createJwtClient,
  driveFileToIndexed,
  DriveMime,
...
import { SyncJobFailed } from "../../errors/index.js"
...
import { parseMail } from "./gmail/index.js"
import {} from "../../search/types.js"
import { GaxiosError } from "gaxios";
const Logger = getLogger(Subsystem.Integrations).child({ module: "google" });
const getDocumentOrSpreadsheet = async (docId) => {
    try {
        const doc = await getDocumentOrNull(fileSchema, docId);
        if (!doc) {
            Logger.error(`Found no document with ${docId}, checking for spreadsheet with ${docId}_0`);
            const sheetsForSpreadSheet = await getDocumentOrNull(fileSchema, `${docId}_0`);
...
const deleteUpdateStatsForGoogleSheets = async (docId, client, stats) => {
    ...
    try {
        ...
        if (spreadsheet) {
            ...
            const spreadSheetFromVespa = await getDocumentOrNull(fileSchema, `${spreadsheetId}_0`);
            if (spreadSheetFromVespa) {
                const metadata = JSON.parse(
                //@ts-ignore
                spreadSheetFromVespa?.fields?.metadata);
                const totalSheetsFromVespa = metadata?.totalSheets;
                if (totalSheets !== totalSheetsFromVespa &&
        ...
        Logger.error(error, `Error in deleteUpdateStatsForGoogleSheets function, but continuing sync engine execution.`);
    }
};
const deleteWholeSpreadsheet = async (docFields, docId, stats, email) => {
    ...
    for (let sheetIndex = 0; sheetIndex < totalSheets; sheetIndex++) {
        ...
        try {
            ...
            Logger.error(err, `Error deleting spreadsheet with id ${docId}, but continuing sync engine execution.`);
        }
    }
};
const handleGoogleDriveChange = async (change, client, email) => {
    ...
    if (change.removed) {
        if (docId) {
            try {
                ...
                if (doc) {
                    ...
                    if ((doc?.fields).mimeType === DriveMime.Sheets) {
                        await deleteWholeSpreadsheet(doc?.fields, docId, stats, email);
                    }
                    else {
                        const permissions = doc?.fields?.permissions;
                        if (permissions.length === 1) {
        ...
        try {
                ...
                try {
                    if (file.mimeType &&
                        ...
                        if (doc) {
                            ...
                            Logger.warn(`Could not get document ${docId}, probably does not exist`);
                            stats.added += 1;
                        }
                    }
                    ...
                    if (file.mimeType && MimeMapForContent[file.mimeType]) {
                        if (file.mimeType === DriveMime.PDF) {
                            ...
                            vespaData = await getSheetsFromSpreadSheet(client, file, DriveEntity.Sheets);
                    ...
                    Logger.error(err, `Couldn't add or update document with docId ${docId}, but continuing sync engine execution.`);
                }
            }
            else {
            ...
            Logger.error(err, `Error syncing file with id ${docId}, but continuing sync engine execution.`);
        }
    }
...
const getDriveChanges = async (driveClient, config, oauth2Client) => {
    try {
        const response = await retryWithBackoff(() => driveClient.changes.list({ pageToken: config.driveToken }), `Fetching drive changes with pageToken ${config.driveToken}`, Apps.GoogleDrive, 0, oauth2Client);
        ...
        if (error instanceof GaxiosError) {
            Logger.error(`GaxiosError while fetching drive changes, but continuing sync engine execution.: status ${error.response?.status}, ` +
                `statusText: ${error.response?.statusText}, data: ${JSON.stringify(error.response?.data)}`);
        }
        else if (error instanceof Error) {
            Logger.error(`Unexpected error while fetching drive changes, but continuing sync engine execution.: ${error.message}`);
        }
        else {
            Logger.error(`An unknown error occurred while fetching drive changes, but continuing sync engine execution.`);
        }
        return null;
    }
};
...
// return early
export const handleGoogleOAuthChanges = async (boss, job) => {
    ...
    for (const syncJob of syncJobs) {
        ...
        try {
            ...
            const driveChanges = await getDriveChanges(driveClient, config, oauth2Client);
            ...
            if (changes?.length &&
                ...
                for (const change of changes) {
                    try {
                        ...
                        Logger.error(err, `Error syncing drive change, but continuing sync engine execution.`);
                    }
                }
            }
            ...
            do {
                try {
                    const response = await retryWithBackoff(() => peopleService.people.connections.list({
                    ...
                    }), `Fetching contacts changes with syncToken ${config.contactsToken}`, Apps.GoogleDrive, 0, oauth2Client);
                    contactsToken = response.data.nextSyncToken ?? contactsToken;
                    nextPageToken = response.data.nextPageToken ?? "";
                    if (response.data.connections) {
                        let changeStats = await syncContacts(peopleService, response.data.connections, user.email, GooglePeopleEntity.Contacts, oauth2Client);
                        stats = mergeStats(stats, changeStats);
                        changesExist = true;
                    }
                }
                catch (err) {
                    Logger.error(err, `Error syncing contacts, but continuing sync engine execution.`);
                    break;
                }
            } while (nextPageToken);
            // reset
            ...
            do {
                try {
                    const response = await retryWithBackoff(() => peopleService.otherContacts.list({
                        pageSize: 1000,
                    ...
                    }), `Fetching other contacts changes with syncToken ${otherContactsToken}`, Apps.GoogleDrive, 0, oauth2Client);
                    otherContactsToken = response.data.nextSyncToken ?? otherContactsToken;
                    nextPageToken = response.data.nextPageToken ?? "";
                    if (response.data.otherContacts) {
                        let changeStats = await syncContacts(peopleService, response.data.otherContacts, user.email, GooglePeopleEntity.OtherContacts);
                        stats = mergeStats(stats, changeStats);
                        changesExist = true;
                    }
                }
                catch (err) {
                    Logger.error(err, `Error syncing other contacts, but continuing sync engine execution.`);
                    break;
                }
            } while (nextPageToken);
            if (changesExist) {
                ...
                Logger.info(`Changes successfully synced for Drive: ${JSON.stringify(stats)}`);
            }
            else {
            ...
            Logger.error(error, `Could not successfully complete sync for Google Drive, but continuing sync engine execution.: ${syncJob.id} due to ${errorMessage} :  ${error.stack}`);
            const config = syncJob.config;
            const newConfig = {
                ...config,
                lastSyncedAt: config.lastSyncedAt.toISOString(),
            };
    ...
    for (const syncJob of gmailSyncJobs) {
        try {
            ...
            let { historyId, stats, changesExist } = await handleGmailChanges(gmail, config.historyId, syncJob.id, syncJob.email, oauth2Client);
            if (changesExist) {
                ...
                Logger.info(`Changes successfully synced for Gmail: ${JSON.stringify(stats)}`);
            }
            else {
            ...
            const errorMessage = getErrorMessage(error);
            Logger.error(error, `Could not successfully complete Oauth sync for Gmail, but continuing sync engine execution: ${syncJob.id} due to ${errorMessage} ${error.stack}`);
            const config = syncJob.config;
            const newConfig = {
                ...config,
                lastSyncedAt: config.lastSyncedAt.toISOString(),
    ...
    const gCalEventSyncJobs = await getAppSyncJobs(db, Apps.GoogleCalendar, AuthType.OAuth);
    for (const syncJob of gCalEventSyncJobs) {
        ...
        try {
            ...
            let { eventChanges, stats, newCalendarEventsSyncToken, changesExist } = await handleGoogleCalendarEventsChanges(calendar, config.calendarEventsToken, syncJob.email, oauth2Client);
            if (changesExist) {
                // update the change token
                ...
                Logger.info(`Changes successfully synced for Google Calendar Events: ${JSON.stringify(stats)}`);
            }
            else {
            ...
            const errorMessage = getErrorMessage(error);
            Logger.error(error, `Could not successfully complete Oauth sync for Google Calendar, but continuing sync engine execution: ${syncJob.id} due to ${errorMessage} ${error.stack}`);
            const config = syncJob.config;
            const newConfig = {
                ...config,
                lastSyncedAt: config.lastSyncedAt.toISOString(),
...
const insertEventIntoVespa = async (event) => {
    try {
        const { baseUrl, joiningUrl } = getJoiningLink(event);
        const { attendeesInfo, attendeesEmails, attendeesNames } = getAttendeesOfEvent(event.attendees ?? []);
        const { attachmentsInfo, attachmentFilenames } = getAttachments(event.attachments ?? []);
        ...
        Logger.error(e, `Error inserting Calendar event with id ${event?.id} into Vespa`);
    }
};
const maxCalendarEventChangeResults = 2500;
const handleGoogleCalendarEventsChanges = async (calendar, syncToken, userEmail, oauth2Client) => {
    let changesExist = false;
    const stats = newStats();
    let nextPageToken = "";
    // will be returned in the end
    let newSyncTokenCalendarEvents = "";
    let eventChanges = [];
    try {
        do {
            const res = await retryWithBackoff(() => calendar.events.list({
                ...
                syncToken,
                fields: eventFields,
            }), `Fetching calendar events changes with syncToken ${syncToken}`, Apps.GoogleCalendar, 0, oauth2Client);
                ...
                if (docId && eventChange.status === "cancelled") {
                    ...
                    try {
                        ...
                        if (!event) {
                            ...
                            Logger.error(`Found no document with ${docId}, checking for event with ${eventId}`);
                            // Update this event and add a instanceDateTime cancelledInstances property
                            try {
                                const eventFromVespa = await getDocumentOrNull(eventSchema, eventId);
                                if (eventFromVespa) {
                                    const oldCancelledInstances = eventFromVespa?.fields
                                        ?.cancelledInstances ?? [];
                                    if (!oldCancelledInstances?.includes(instanceDateTime)) {
                                        ...
                                        if (eventFromVespa) {
                                            await UpdateEventCancelledInstances(eventSchema, eventId, newCancelledInstances);
                                            stats.updated += 1;
                                    ...
                                    Logger.error(`Can't find event to delete, probably doesn't exist`);
                                }
                            }
                            catch (error) {
                                Logger.error(error, `Error deleting Calendar event, but continuing sync engine execution.`);
                            }
                            finally {
                                continue;
                            }
                        }
                        const permissions = event?.fields?.permissions;
                        if (permissions?.length === 1) {
                            // remove it
                            try {
                                // also ensure that we are that permission
                                ...
                                Logger.error(`Couldn't delete document with id ${docId}, but continuing sync engine execution.`);
                            }
                        }
                        else {
                        ...
                        Logger.error(err, `Error getting document, but continuing sync engine execution: ${err.message} ${err.stack}`);
                    }
                }
                else if (docId) {
                    let event = null;
        ...
        Logger.error(err, `Error handling Calendar event changes, but continuing sync engine execution.`);
        return {
            eventChanges,
            stats,
            newCalendarEventsSyncToken: newSyncTokenCalendarEvents,
...
const handleGmailChanges = async (gmail, historyId, syncJobId, userEmail, client) => {
    let changesExist = false;
    const stats = newStats();
    let nextPageToken = "";
    ...
    try {
        do {
            const res = await retryWithBackoff(() => gmail.users.history.list({
                userId: "me",
                startHistoryId: historyId,
                maxResults: maxChangeResults,
                pageToken: nextPageToken,
            }), `Fetching gmail changes with historyId ${historyId}`, Apps.Gmail, 0, client);
                ...
                for (const history of res.data.history) {
                    if (history.messagesAdded) {
                        for (const { message } of history.messagesAdded) {
                            try {
                                const msgResp = await retryWithBackoff(() => gmail.users.messages.get({
                                    userId: "me",
                                    id: message?.id,
                                    format: "full",
                                }), `Fetching gmail email with id ${message?.id}`, Apps.Gmail, 0, client);
                                await insert(await parseMail(msgResp.data, gmail, userEmail, client), mailSchema);
                                stats.added += 1;
                                changesExist = true;
                            }
                            catch (error) {
                                // Handle errors if the message no longer exists
                                Logger.error(error, `Failed to fetch added message ${message?.id} in historyId ${history.id}: ${error}, but continuing sync engine execution.`);
                            }
                        }
                    }
                    if (history.messagesDeleted) {
                        for (const { message } of history.messagesDeleted) {
                            try {
                                const mailMsg = await getDocumentOrNull(mailSchema, message?.id);
                                if (mailMsg) {
                                    const permissions = mailMsg?.fields
                                        ?.permissions;
                                    if (permissions?.length === 1) {
                                        ...
                                        const newPermissions = permissions?.filter((v) => v !== userEmail);
                                        await UpdateDocumentPermissions(mailSchema, message?.id, newPermissions);
                                    }
                                    stats.removed += 1;
                                    changesExist = true;
                                }
                                ...
                                Logger.error(error, `Failed to delete message ${message?.id} in historyId ${history.id}: ${error}, but continuing sync engine execution.`);
                            }
                        }
                    }
                    if (history.labelsAdded) {
                        for (const { message, labelIds } of history.labelsAdded) {
                            try {
                                const mailMsg = await getDocumentOrNull(mailSchema, message?.id);
                                if (mailMsg) {
                                    let labels = mailMsg?.fields?.labels || [];
                                ...
                                Logger.error(error, `Failed to add labels to message ${message?.id} in historyId ${history.id}: ${error}, but continuing sync engine execution.`);
                            }
                        }
                    }
                    if (history.labelsRemoved) {
                        for (const { message, labelIds } of history.labelsRemoved) {
                            try {
                                ...
                                if (mailMsg) {
                                    let labels = mailMsg?.fields?.labels || [];
                                    labels = labels.filter((label) => !(labelIds || []).includes(label));
                                    await UpdateDocument(mailSchema, message?.id, { labels });
                                    stats.updated += 1;
                                    changesExist = true;
                                }
                                ...
                                Logger.error(error, `Failed to remove labels from message ${message?.id} in historyId ${history.id}: ${error}, but continuing sync engine execution.`);
                            }
                        }
                    }
                }
        ...
        if (error?.code === 404 ||
            ...
            Logger.error(error, `Invalid historyId ${historyId}, but continuing sync engine execution: ${error}`);
            return { stats, historyId: newHistoryId, changesExist };
        }
        else {
            Logger.error(error, `Error handling Gmail changes, but continuing sync engine execution.`);
            return { stats, historyId: newHistoryId, changesExist };
        }
    }
    return { historyId: newHistoryId, stats, changesExist };
};
const syncContacts = async (client, contacts, email, entity, oauth2Client) => {
    const stats = newStats();
    ...
    for (const contact of connections) {
        try {
            ...
            if (contact.metadata?.deleted && contact.resourceName) {
                ...
                if (contact.resourceName) {
                    if (entity === GooglePeopleEntity.Contacts) {
                        ...
                        const contactResp = await retryWithBackoff(() => client.people.get({
                            resourceName: contact.resourceName,
                            personFields: contactKeys.join(","),
                        }), `Fetching contact with resourceName ${contact.resourceName}`, Apps.GoogleDrive, 0, oauth2Client);
            ...
            Logger.error(e, `Error in syncing contact, but continuing sync engine execution.`);
        }
    }
    return stats;
};
export const handleGoogleServiceAccountChanges = async (boss, job) => {
    ...
    for (const syncJob of syncJobs) {
        ...
        try {
            ...
            const user = await getUserById(db, connector.userId);
            const serviceAccountKey = JSON.parse(connector.credentials);
            let jwtClient = createJwtClient(serviceAccountKey, syncJob.email);
            const driveClient = google.drive({ version: "v3", auth: jwtClient });
            const config = syncJob.config;
            // TODO: add pagination for all the possible changes
            const { changes, newStartPageToken } = (await retryWithBackoff(() => driveClient.changes.list({ pageToken: config.driveToken }), `Fetching drive changes with pageToken ${config.driveToken}`, Apps.GoogleDrive)).data;
            // there are changes
            // Potential issues:
            // we remove the doc but don't update the syncJob
            // leading to us trying to remove the doc again which throws error
            ...
            try {
                do {
                    const response = await retryWithBackoff(() => peopleService.people.connections.list({
                        ...
                        pageSize: 1000, // Adjust the page size based on your quota and needs
                        pageToken: nextPageToken, // Use the nextPageToken for pagination
                    }), `Fetching contacts changes with syncToken ${config.contactsToken}`, Apps.GoogleDrive);
                    if (response.data.connections && response.data.connections.length) {
                        Logger.info(`About to update ${response.data.connections.length} contacts`);
                        let changeStats = await syncContacts(peopleService, response.data.connections, user.email, GooglePeopleEntity.Contacts);
                        stats = mergeStats(stats, changeStats);
                        changesExist = true;
                    }
                } while (nextPageToken);
                ...
                if (error.response &&
                    ...
                    Logger.warn("This is an error that is not yet implemented, it requires a full sync of the contacts api, but continuing sync engine execution for now.");
                }
                else {
                    Logger.error(error, `Error syncing contacts, but continuing sync engine execution.`);
                }
            }
            // reset
            nextPageToken = "";
            try {
                do {
                    ...
                    }), `Fetching other contacts changes with syncToken ${otherContactsToken}`, Apps.GoogleDrive);
                    otherContactsToken = response.data.nextSyncToken ?? otherContactsToken;
                    if (response.data.otherContacts &&
                        response.data.otherContacts.length) {
                        Logger.info(`About to update ${response.data.otherContacts.length} other contacts`);
                        let changeStats = await syncContacts(peopleService, response.data.otherContacts, user.email, GooglePeopleEntity.OtherContacts);
                        stats = mergeStats(stats, changeStats);
                        changesExist = true;
                    }
                ...
                if (error.response &&
                    ...
                    error.message ===
                        "Sync token is expired. Clear local cache and retry call without the sync token.") {
                    Logger.warn("This is an error that is not yet implemented, it requires a full sync of the other contacts api, but continuing sync engine execution for now.");
                }
                else {
                    Logger.error(error, `Error syncing other contacts, but continuing sync engine execution for now.`);
                }
            }
            if (changesExist) {
                const newConfig = {
            ...
            const errorMessage = getErrorMessage(error);
            Logger.error(error, `Could not successfully complete ServiceAccount sync for Google Drive, but continuing sync engine execution: ${syncJob.id} due to ${errorMessage} ${error.stack}`);
            const config = syncJob.config;
            const newConfig = {
                ...config,
                lastSyncedAt: config.lastSyncedAt.toISOString(),
    ...
    const gmailSyncJobs = await getAppSyncJobs(db, Apps.Gmail, AuthType.ServiceAccount);
    let stats = newStats();
    for (const syncJob of gmailSyncJobs) {
        try {
            const connector = await getConnector(db, syncJob.connectorId);
            ...
            let { historyId, stats, changesExist } = await handleGmailChanges(gmail, config.historyId, syncJob.id, syncJob.email);
            if (changesExist) {
                // update the change token
                config.historyId = historyId;
                await db.transaction(async (trx) => {
            ...
            const errorMessage = getErrorMessage(error);
            Logger.error(error, `Could not successfully complete ServiceAccount sync for Gmail, but continuing sync engine execution: ${syncJob.id} due to ${errorMessage} ${error.stack}`);
            const config = syncJob.config;
            const newConfig = {
                ...config,
                lastSyncedAt: config.lastSyncedAt.toISOString(),
    ...
    // For Calendar Events Sync
    const gCalEventSyncJobs = await getAppSyncJobs(db, Apps.GoogleCalendar, AuthType.ServiceAccount);
    for (const syncJob of gCalEventSyncJobs) {
        try {
            const connector = await getConnector(db, syncJob.connectorId);
            const serviceAccountKey = JSON.parse(connector.credentials);
            ...
            let { eventChanges, stats, newCalendarEventsSyncToken, changesExist } = await handleGoogleCalendarEventsChanges(calendar, config.calendarEventsToken, syncJob.email);
            if (changesExist) {
                // update the change token
                config.calendarEventsToken = newCalendarEventsSyncToken;
                await db.transaction(async (trx) => {
            ...
            Logger.error(error, `Could not successfully complete ServiceAccount sync for Google Calendar, but continuing sync engine execution: ${syncJob.id} due to ${errorMessage} ${error.stack}`);
            const config = syncJob.config;
            const newConfig = {
                ...config,
                lastSyncedAt: config.lastSyncedAt.toISOString(),
