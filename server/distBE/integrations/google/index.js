...
import {
  ...
  getFile,
  toPermissionsList,
} from "../../integrations/google/utils.js"
import { getLogger } from "../../logger/index.js"
import { CalendarEntity, eventSchema } from "../../search/types.js"
import {
  UserListingError,
...
import { handleGmailIngestion } from "../../integrations/google/gmail/index.js"
import pLimit from "p-limit"
import { GoogleDocsConcurrency } from "./config.js"
import {
  getProgress,
  markUserComplete,
  oAuthTracker,
...
import { getOAuthProviderByConnectorId } from "../../db/oauthProvider.js"
...
const gmailWorker = new Worker(new URL("./gmail-worker.js", import.meta.url))
export const listUsersByEmails = async (admin, emails) => {
  const users = []
  try {
    ...
    Logger.error(error, `Error fetching users: ${error} ${error.stack}`)
...
export const listUsers = async (admin, domain) => {
  let users = []
  ...
  try {
    ...
    Logger.error(error, `Error listing users: ${error} ${error.stack}`)
    throw new UserListingError({
      cause: error,
      integration: Apps.GoogleWorkspace,
      entity: "user",
...
export const syncGoogleWorkspace = async (boss, job) => {
  ...
  try {
    ...
    const subject = connector.subject
    ...
    Logger.error(
      error,
      `Could not sync Google workspace: , ${errorMessage} ${error.stack}`,
    )
...
export const getTextFromEventDescription = (description) => {
  return htmlToText.convert(description, { wordwrap: 130 })
}
const getBaseUrlFromUrl = (url) => {
  try {
    if (url) {
...
export const getJoiningLink = (event) => {
...
export const getAttendeesOfEvent = (allAttendes) => {
...
export const getAttachments = (allAttachments) => {
...
export const getUniqueEmails = (permissions) => {
...
const insertCalendarEvents = async (client, userEmail) => {
  let nextPageToken = ""
...
export const handleGoogleOAuthIngestion = async (data) => {
  ...
  try {
    ...
    const providers = await getOAuthProviderByConnectorId(db, data.connectorId)
    ...
    const { startPageToken } = (await driveClient.changes.getStartPageToken())
      .data
    if (!startPageToken) {
    ...
    Logger.error(
      error,
      `could not finish job successfully: ${error.message} ${error.stack}`,
      error,
    )
    // await db.transaction(async (trx) => {
    await db
...
export const handleGoogleServiceAccountIngestion = async (data) => {
...
export const getPresentationToBeIngested = async (presentation, client) => {
...
const googleSlidesVespa = async (client, presentationMetadata, connectorId) => {
  // sendWebsocketMessage(
  ...
  for (const presentation of presentationMetadata) {
    try {
      ...
      Logger.error(
        error,
        `Error getting slides: ${error} ${error.stack}`,
        error,
      )
      continue
    }
...
const insertFilesForUser = async (googleClient, userEmail, connector) => {
  try {
    ...
    for await (const pageFiles of iterator) {
      ...
      const [documents, slides, sheets] = await Promise.all([
      ...
      const driveFiles = await driveFilesToDoc(googleClient, rest)
      let allFiles = [...driveFiles, ...documents, ...slides, ...sheets].map(
        (v) => {
          v.permissions = toPermissionsList(v.permissions, userEmail)
          return v
        },
    ...
    Logger.error(
      error,
      `Could not insert files for user: ${errorMessage} ${error.stack}`,
    )
  }
}
export const cleanSheetAndGetValidRows = (allRows) => {
  ...
  // Put "" string in them
  const processedRows = rowsWithData.map((row) =>
    row.length < noOfCols
...
export const getAllSheetsFromSpreadSheet = async (
  ...
  for (let i = 0; i < gridSheets.length; i += batchSize) {
    ...
    try {
      ...
      Logger.error(
        error,
        `Failed to fetch sheets '${ranges.join(", ")}' from spreadsheet: ${error.message}`,
      )
      continue
    }
  }
...
export const getSpreadsheet = async (sheets, id, client) => {
  try {
...
export const getSheetsListFromOneSpreadsheet = async (
  ...
  const sheetsArr = []
  try {
    const spreadSheetData = await getSpreadsheet(sheets, spreadsheet.id, client)
    if (spreadSheetData) {
...
const googleSheetsVespa = async (
  ...
  for (const spreadsheet of spreadsheetsMetadata) {
    try {
      ...
      Logger.error(
        error,
        `Error getting sheet files: ${error} ${error.stack}`,
        error,
      )
...
export const downloadPDF = async (drive, fileId, fileName, client) => {
  ...
  const pdfPromises = pdfsMetadata.map((pdf) =>
    limit(async () => {
      ...
      try {
        ...
        Logger.error(
          error,
          `Error getting PDF files: ${error} ${error.stack}`,
          error,
        )
...
const insertUsersForWorkspace = async (users) => {
  for (const user of users) {
    const currentOrg =
      user.organizations?.find((org) => !org.endDate) || user.organizations?.[0]
    const preferredLanguage =
      user.languages?.find((lang) => lang.preference === "preferred")
...
const listAllContacts = async (client) => {
  ...
  const otherContacts = []
  // will be returned in the end
  let newSyncTokenContacts = ""
  let newSyncTokenOtherContacts = ""
  do {
    const response = await retryWithBackoff(
      () =>
...
export const insertContact = async (contact, entity, owner) => {
...
const insertContactsToVespa = async (contacts, otherContacts, owner) => {
  try {
    ...
    if (error instanceof ErrorInsertingDocument) {
      Logger.error(error, `Could not insert contact: ${error.stack}`)
      throw error
    } else {
      Logger.error(
        error,
        `Error mapping contact: ${error} ${error.stack}`,
        error,
      )
      throw new ContactMappingError({
...
const sendWebsocketMessage = (message, connectorId) => {
  const ws = wsConnections.get(connectorId)
  if (ws) {
    ws.send(JSON.stringify({ message }))
  }
}
export const googleDocsVespa = async (client, docsMetadata, connectorId) => {
  // sendWebsocketMessage(
  ...
  const docsPromises = docsMetadata.map((doc) =>
    limit(async () => {
      try {
        const docResponse = await retryWithBackoff(
          () =>
            docs.documents.get({
              documentId: doc.id,
            }),
        ...
        Logger.error(
          error,
          `Error processing Google Doc: ${errorMessage} ${error.stack}`,
        )
        return null
      }
    }),
  )
  const docsList = await Promise.all(docsPromises)
  return docsList.filter((doc) => doc !== null)
}
export const driveFilesToDoc = async (client, rest) => {
  let results = []
...
export async function getGmailCounts(client) {
...
// Count Drive Files
export async function countDriveFiles(client) {
  const drive = google.drive({ version: "v3", auth: client })
  let fileCount = 0
  let nextPageToken
  do {
