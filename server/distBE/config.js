...
let defaultFastModel = "";
let defaultBestModel = "";
let AwsAccessKey = "";
let AwsSecretKey = "";
let OpenAIKey = "";
let OllamaModel = "";
...
if (process.env["FAST_MODEL_REASONING"] &&
    process.env["FAST_MODEL_REASONING"] === "true") {
    fastModelReasoning = true;
}
let serviceAccountWhitelistedEmails = [];
if (process.env["SERVICE_ACCOUNT_WHITELISTED_EMAILS"]) {
    serviceAccountWhitelistedEmails = process.env["SERVICE_ACCOUNT_WHITELISTED_EMAILS"]
        .split(",")
        .map((v) => v.trim());
}
export default {
