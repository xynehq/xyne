...
export const checkAndReadFile = async (path) => {
    try {
...
export const setCookieByEnv = (c, CookieName, jwtToken, opts) => {
...
export const getRelativeTime = (oldTimestamp) => {
 ...
 */
export const retryWithBackoff = async (fn, context, app, retries = 0, googleOauth2Client) => {
    try {
        ...
        // error.code === 403
        const isGoogleTimeoutError = error.code === "ETIMEDOUT" ||
        ...
        if ((isQuotaError || isGoogleTimeoutError) && retries < MAX_RETRIES) {
            ...
            Logger.info(`[${context}] Quota error. Retrying after ${waitTime.toFixed(0)}ms (Attempt ${retries + 1}/${MAX_RETRIES})`);
            ...
            if (IsGoogleApp(app)) {
                ...
                return retryWithBackoff(fn, context, app, retries + 1, googleOauth2Client);
            }
            else {
            ...
            Logger.error(`[${context}] Failed after ${retries} retries: ${error.message}`);
            throw error; // Rethrow error if retries are exhausted or not quota-related
        }
    }
};
export const splitGroupedCitationsWithSpaces = (text) => {
...
export const IsGoogleApp = (app) => {
    return (app === Apps.GoogleDrive ||
        app === Apps.Gmail ||
