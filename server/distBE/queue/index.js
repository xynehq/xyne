...
} from "../integrations/google/sync.js"
import { checkDownloadsFolder } from "../integrations/google/utils.js"
...
export const setupServiceAccountCronjobs = async () => {
    await boss.schedule(SyncServiceAccountSaaSQueue, Every10Minutes, {}, { retryLimit: 0, expireInHours: JobExpiryHours });
    await boss.schedule(SyncGoogleWorkspace, Every6Hours, {}, { retryLimit: 0, expireInHours: JobExpiryHours });
};
const initWorkers = async () => {
    ...
    await boss.work(SaaSQueue, async ([job]) => {
        ...
        if (jobData.app === Apps.GoogleDrive &&
        ...
        else if (jobData.app === Apps.GoogleDrive &&
    ...
    // do not retry
    await boss.schedule(SyncOAuthSaaSQueue, Every10Minutes, {}, { retryLimit: 0, expireInHours: JobExpiryHours });
    await boss.schedule(CheckDownloadsFolderQueue, EveryWeek, {}, { retryLimit: 0, expireInHours: JobExpiryHours });
    ...
    await boss.work(SyncOAuthSaaSQueue, async ([job]) => {
        try {
            ...
            Logger.error(error, `Unhandled Error while syncing OAuth SaaS ${errorMessage} ${error.stack}`);
        }
    });
...
boss.on("error", (error) => {
    console.error(`Queue error: ${error} ${error.stack}`);
});
