...
if (!args.length) {
    throw new Error("Model dimensions are missing. Please provide the model dimensions as an argument.");
}
if (Number.isNaN(parseInt(args[0]))) {
    throw new Error("invalid dimensions");
}
const replaceDimensions = (filePath, modelDimension) => {
    try {
...
const replaceModelDIMS = (filePaths, modelDimension) => {
    filePaths.forEach((p) => {
        try {
...
const getSchemaDefinitionPaths = (directory) => {
    return fs
