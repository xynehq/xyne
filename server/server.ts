...
async function loadWebhooksFromDatabase() {
  try {
...
async function executeWorkflowFromWebhook(
  ...
  try {
    ...
    startWorkflowExecution(execution.id, stepExecution.id, webhookData)
      .catch(error => {
...
app.all("/webhook/*", async (c) => {
  ...
  try {
    executionId = await executeWorkflowFromWebhook(
...
app.get("/webhook-api/reload", async (c) => {
  try {
    await loadWebhooksFromDatabase()
    return c.json({ 
      success: true, 
