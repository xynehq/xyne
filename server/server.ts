...
export type Variables = JwtVariables & {
    mcpClients: Record<number, Client>
}
...
// this will become DB/redis.
const userClientsMap: Record<number, Record<string, Client>> = {}

app.use('*', async (c, next) => {
    const { userId } = c.get('jwtPayload')

    if (!userClientsMap[userId]) {
        userClientsMap[userId] = {}
        // const clientConfigs = await db.getUserMcpClients(userId)
        // for (const config of clientConfigs) {
        //   userClientsMap[userId][config.name] = new Client(config)
        // }
    }
    c.set('mcpClients', userClientsMap[userId])
    await next()
})
...
export const AppRoutes = app
    ...
    .get("/message/create/tools", zValidator("query", messageSchema), MessageWithToolsApi)
    .get(
        "/message/retry",
