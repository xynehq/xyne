import { pgTable, uniqueIndex, foreignKey, unique, serial, integer, text, timestamp, jsonb, pgEnum } from "drizzle-orm/pg-core"
  import { sql } from "drizzle-orm"

export const appType = pgEnum("app_type", ['google-workspace', 'google-drive', 'notion', 'gmail'])
export const authType = pgEnum("auth_type", ['oauth', 'service_account', 'custom', 'api_key'])
export const connectorType = pgEnum("connector_type", ['SaaS', 'Database', 'Api', 'File', 'Website'])
export const role = pgEnum("role", ['User', 'TeamLeader', 'Admin', 'SuperAdmin'])
export const status = pgEnum("status", ['connected', 'connecting', 'failed', 'not-connected'])
export const syncStatus = pgEnum("sync_status", ['NotStarted', 'Started', 'Failed', 'Successful'])
...
export const users = pgTable("users", {
	...
	deletedAt: timestamp("deleted_at", { withTimezone: true, mode: 'string' }).default('1970-01-01 00:00:00+00').notNull(),
...
(table) => {
	return {
		...
		oauthProvidersExternalIdUnique: unique("oauth_providers_external_id_unique").on(table.externalId),
	}
});
...
export const syncJobs = pgTable("sync_jobs", {
	...
	lastRanOn: timestamp("last_ran_on", { withTimezone: true, mode: 'string' }).defaultNow(),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	updatedAt: timestamp("updated_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
...
(table) => {
	return {
		...
		syncJobsExternalIdUnique: unique("sync_jobs_external_id_unique").on(table.externalId),
	}
});
...
(table) => {
	return {
		workspacesDomainUnique: unique("workspaces_domain_unique").on(table.domain),
		workspacesCreatedByUnique: unique("workspaces_created_by_unique").on(table.createdBy),
		workspacesExternalIdUnique: unique("workspaces_external_id_unique").on(table.externalId),
	}
});
...
(table) => {
	return {
		...
		connectorsWorkspaceIdUserIdAppTypeAuthTypeUnique: unique("connectors_workspace_id_user_id_app_type_auth_type_unique").on(table.workspaceId, table.userId, table.authType, table.appType),
		connectorsExternalIdUnique: unique("connectors_external_id_unique").on(table.externalId),
	}
});