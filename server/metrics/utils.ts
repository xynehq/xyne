...
export const updateMetricsFromThread = ({
  ...
  totalIngestedMails.inc({ email, account_type: AuthType.ServiceAccount, status: OperationStatus.Success }, messageCount)
  totalAttachmentIngested.inc({ email, account_type: AuthType.ServiceAccount, status: OperationStatus.Success }, attachmentCount)
  ingestionMailErrorsTotal.inc({ email, account_type: AuthType.ServiceAccount, status: OperationStatus.Failure }, failedMessages)
  totalAttachmentError.inc({ email, account_type: AuthType.ServiceAccount, status: OperationStatus.Failure }, failedAttachments)
  ...
  totalIngestedFiles.inc({ mime_type: DriveMime.Slides, status: OperationStatus.Success, email, file_type: DriveEntity.Slides }, slidesCount)
  totalIngestedFiles.inc({ mime_type: "application/vnd.google-apps.file", status: OperationStatus.Success, email, file_type: DriveEntity.Misc }, fileCount)
  totalDriveFilesToBeIngested.inc({ email, file_type: DriveEntity.Misc, status: OperationStatus.Success }, totalDriveFiles)
  blockedFilesTotal.inc({ email, file_type: DriveEntity.PDF, mime_type: DriveMime.PDF, status: OperationStatus.Cancelled }, blockedPdfs)
