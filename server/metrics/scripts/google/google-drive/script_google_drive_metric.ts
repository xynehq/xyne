...
export const totalIngestedFilesScript = new Counter({
  name: "google_drive_ingested_total",
  help: "Total number of ingested files",
  labelNames: ["mime_type", "status", "email", "file_type"],
})
metricRegister.registerMetric(totalIngestedFilesScript) // register this metric function in the registry

// Collects metrics for errors in file ingestion
export const ingestionErrorsTotalScript = new Counter({
...
metricRegister.registerMetric(ingestionErrorsTotalScript)

//Collects metics for the blocked files
export const blockedFilesTotalScript = new Counter({
...
metricRegister.registerMetric(blockedFilesTotalScript)

// Collects metrics for the duration of extraction of a file's content
export const extractionDurationScript = new Histogram({
...
metricRegister.registerMetric(extractionDurationScript)

// Collects metrics for size of content-extracted files [ Ranges from - 10KB to 1GB]
export const contentFileSizeScript = new Histogram({
...
metricRegister.registerMetric(contentFileSizeScript)

// Collect the errors during file extraction
export const fileExtractionErrorsTotalScript = new Counter({
...
metricRegister.registerMetric(fileExtractionErrorsTotalScript)

// Collect the total number of extracted files
export const totalExtractedFilesScript = new Counter({
  name: "google_drive_files_extracted_total",
  help: "Total number of extracted files",
  labelNames: ["mime_type", "status", "email", "file_type"],
})
metricRegister.registerMetric(totalExtractedFilesScript)

// Collects the metrics for the total time required for extracting a specific type of file
export const totalDurationForFileExtractionScript = new Histogram({
...
metricRegister.registerMetric(totalDurationForFileExtractionScript)

export const totalDriveFilesToBeIngestedScript = new Counter({
  name: "total_files_to_be_inserted",
  help: "Total number of drive files to be inserted",
  labelNames: ["email", "file_type", "status"],
})

metricRegister.registerMetric(totalDriveFilesToBeIngestedScript)
