...
export const ExecuteWorkflowWithInputApi = async (c: Context) => {
  try {
          ...
          if (field.type === "file") {
            ...
            if (file instanceof File) {
              try {
                ...
                // Don't throw here - let the workflow be marked as failed properly
                // The error will be thrown after database updates are complete
              }
            }
          }
        }
    ...
    if (hasUploadError) {
      ...
      // Now throw the error after all database updates are complete
      throw new HTTPException(400, {
        message: `File upload failed for ${processedFormData._uploadError.field}: ${processedFormData._uploadError.error}`,
      })
    }
    }
