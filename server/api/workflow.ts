...
export const ExecuteWorkflowWithInputApi = async (c: Context) => {
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      const entries = Array.from((formData as any).entries()) as [
        string,
        FormDataEntryValue,
      ][]
...
const executeAutomatedWorkflowSteps = async (
  ...
  try {
    ...
    // Check if workflow is completed using improved completion logic
    await checkAndUpdateWorkflowCompletion(executionId)
...
export const GetWorkflowExecutionStatusApi = async (c: Context) => {
  try {
    ...
    // Get execution with metadata for detailed status
    const execution = await db
      .select({
    ...
    const progressInfo = (executionData.metadata as any)?.executionProgress || {
      totalSteps: 0,
      executedSteps: 0,
      completedSteps: 0,
      failedSteps: 0,
      activeSteps: 0,
      manualStepsAwaitingInput: 0,
      lastUpdated: null
    }
    ...
    return c.json({
      ...
      isComplete: executionData.status === "completed" || executionData.status === "failed"
    })
  } catch (error) {
...
export const SubmitWorkflowFormApi = async (c: Context) => {
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      const entries = Array.from((multipartData as any).entries()) as [
        string,
        FormDataEntryValue,
      ][]
      for (const [key, value] of entries) {
    ...
    // Initialize variables - currentStepExecution is from multipart block if it exists
    let stepExecution: any = null
    let formTool = null

    // Check if we have currentStepExecution from multipart processing
    if (contentType.includes("multipart/form-data")) {
      // For multipart, we already have the step execution
      const stepExecutions = await db
        .select()
        .from(workflowStepExecution)
        .where(eq(workflowStepExecution.id, stepId))
    ...
    // Check if workflow is completed after form submission
    await checkAndUpdateWorkflowCompletion(stepExecution.workflowExecutionId)
