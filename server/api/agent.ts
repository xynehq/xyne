...
import { insertAgent, getAgentsByUserId, updateAgentByExternalId, getAgentByExternalId, deleteAgentByExternalId } from "@/db/agent"
...
export const createAgentSchema = z.object({ // Keep this for create
...
export const CreateAgentApi = async (c: Context) => {
  try {
    ...
    if (!userAndWorkspace || !userAndWorkspace.user || !userAndWorkspace.workspace) {
    ...
    Logger.error(error, `Create Agent Error: ${errMsg} ${(error as Error).stack}`)
    if (error instanceof z.ZodError) {
      return c.json({ message: "Invalid input", errors: error.flatten().fieldErrors }, 400);
    }
...
export const UpdateAgentApi = async (c: Context) => {
  try {
    ...
    const existingAgent = await getAgentByExternalId(db, agentExternalId, userAndWorkspace.workspace.id)
    ...
    const errMsg = getErrorMessage(error)
    Logger.error(error, `Update Agent Error: ${errMsg} ${(error as Error).stack}`)
    if (error instanceof z.ZodError) {
      return c.json({ message: "Invalid input", errors: error.flatten().fieldErrors }, 400);
    }
...
export const DeleteAgentApi = async (c: Context) => {
  try {
    ...
    return c.json({ message: "Agent deleted successfully", agent: selectPublicAgentSchema.parse(deletedAgent) });

  } catch (error) {
    const errMsg = getErrorMessage(error)
    Logger.error(error, `Delete Agent Error: ${errMsg} ${(error as Error).stack}`)
...
export const ListAgentsApi = async (c: Context) => {
  try {
    ...
    const email = sub
    // @ts-ignore
    const { limit, offset } = c.req.valid("query") as z.infer<typeof listAgentsSchema>
    ...
    const errMsg = getErrorMessage(error)
    Logger.error(error, `List Agents Error: ${errMsg} ${(error as Error).stack}`)
