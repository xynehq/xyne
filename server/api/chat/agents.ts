...
const hasNoAppIntegrations = (agent: SelectAgent | null): boolean => {
  if (!agent?.appIntegrations) return true
  
  if (typeof agent.appIntegrations === 'object') {
    if (Object.keys(agent.appIntegrations).length === 0) return true
    
    return Object.values(agent.appIntegrations).every(config => 
...
const handleAgentWithNoIntegrations = async (c: Context, agentForDb: SelectAgent | null) => {
  const modifiedContext = c  
  if (agentForDb) {
    agentForDb.appIntegrations = {}
  }
  return AgentMessageApiRagOff(modifiedContext)
}
...
export const AgentMessageApi = async (c: Context) => {
  ...
  try {
    ...
    if (agentId && isCuid(agentId)) {
      ...
      // Check if no app integrations are selected and delegate to AgentMessageApiRagOff
      if (hasNoAppIntegrations(agentForDb)) {
        return handleAgentWithNoIntegrations(c, agentForDb)
      }
    }
    const agentIdToStore = agentForDb ? agentForDb.externalId : null
    const userRequestsReasoning = isReasoningEnabled
