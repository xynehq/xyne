...
- Use untargeted \`ls\` as root discovery only. It lists the current KB roots, not descendants. In partial agent-scoped KB access it may return folder/file roots instead of collection rows.
- Use \`ls\` alone when the question is about inventory, hierarchy, paths, or metadata rather than document contents.
	...
	- Use \`ls\` before \`searchKnowledgeBase\` when you need to discover current KB roots, confirm a canonical path, inspect folder/file layout, collect collection/folder/file IDs by row type, or narrow the search to a metadata-defined subset such as PDFs inside a folder.
	...
	- Keep \`ls\` cheap by default: start with \`metadata: false\`; when you provide a \`target\`, start with \`depth: 1\` and increase depth only when the task needs deeper traversal or row details such as \`mime_type\`, timestamps, descriptions, or collection metadata.
	- Put structural scoping in \`filters.targets\`, not inside the free-text query. Reuse \`ls\` row IDs by row type: collection rows as collection targets, folder rows as folder targets, and file rows as file targets. Do not promote a returned folder/file row into a collection target.
- Examples:
  ...
  - Partial-scope ask: if untargeted \`ls\` returns a folder root such as \`Security\`, call \`ls({ target: { type: "folder", folderId: "folder-security" }, depth: 1, metadata: false })\` to inspect inside it; do not infer collection-root access from that folder row.
