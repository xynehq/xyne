...
import { insertChat, updateChatByExternalId } from "@/db/chat"
...
export const MessageApiAgenticMinimal = async (
      ...
      execute: async (_params: any, span?: Span) => {
        ...
        try {
          ...
          const userFragment: MinimalAgentFragment = {
            ...
            content: userCtxObject, // The string generated by userContext()
            source: {
  ...
  **Query Analysis & Tool Selection Strategy:**
  1.  **Classify Intent:** What is the user's primary goal?
      *   GET basic user/system information (e.g., "What's my name?", "What time is it?")?
  ...
  **Action Selection (Choose ONE):**
  ...
  IF Available Context Fragments is 0 (or if current fragments are not sufficient and a new tool call is needed):
    THEN Action MUST BE: **Use a Tool** (Select from the 'Available Tools' list below and provide parameters as JSON).
      *   **Tool Prioritization based on Intent & Keywords:**
            *   If Intent is GET basic user/system information (e.g. "What is my name?", "My email?", "What time is it?", "Who am I?"): **Strongly prefer 'get_user_info'.** This tool requires no parameters.
  ...
  ELSE (Available Context Fragments > 0 AND they are sufficient or max iterations reached):
    Action: **Synthesize Answer** {'tool': 'SYNTHESIZE_ANSWER', 'parameters': {}}
       *   Choose ONLY if:
            *   Enough context exists to fully answer the query (considering intent, quantity, validation). This includes context from 'get_user_info' if it was called and is relevant.
       ...
       *   Do NOT choose if:
            ...
            *   The 'get_user_info' tool was just called and its output is the only fragment, and the query is a direct question for that info (in this case, synthesis should be straightforward).

  Available Tools:
  1. get_user_info: Retrieves basic information about the current user and their environment (name, email, company, current date/time). No parameters needed. This tool does not accept/use 'excludedIds'.
  2. metadata_retrieval: Retrieves a *list* based *purely on metadata/time/type*. Ideal for 'latest'/'oldest'/count and typed items like 'receipts', 'contacts', or 'users'.
  ...
  3. search: Search *content* across all sources. Params: query (req keywords), limit (opt), excludedIds (opt: string[]).
  4. filtered_search: Search *content* within a specific app.
  ...
  5. time_search: Search *content* within a specific time range. Params: query (req keywords), from_days_ago (req), to_days_ago (req), limit (opt), excludedIds (opt: string[]).
