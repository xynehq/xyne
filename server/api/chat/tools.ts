...
function toolError(
  ...
  return {
    result: options?.result || `Error: ${errorMessage}`,
    error: errorMessage,
    contexts: options?.contexts,
    metadata: options?.metadata,
  }
...
async function formatSearchToolResponse(
  ...
  if (children.length === 0) {
    return toolSuccess(`No ${searchContext.searchType || "results"} found.`, {
      contexts: [],
      metadata: {
        query: searchContext.query,
        app: searchContext.app,
        resultCount: 0,
      },
    })
  }
  ...
  return toolSuccess(summaryText, {
    contexts: fragments,
    metadata: {
      query: searchContext.query,
      app: searchContext.app,
...
async function executeVespaSearch(
  ...
  if (!email) {
    ...
    execSpan?.end()
    return toolError("Missing email", { result: errorMsg, contexts: [] })
  }
  ...
  if (query && query.trim() !== "") {
    if (agentAppEnums && agentAppEnums.length > 0) {
      ...
      if (invalidApps.length > 0) {
        ...
        execSpan?.setAttribute("error", errorMsg)
        execSpan?.end()
        return toolError(errorMsg, { contexts: [] })
      }
    ...
    execSpan?.setAttribute("error", errorMsg)
    execSpan?.end()
    return toolError("Invalid search parameters", {
      result: errorMsg,
      contexts: [],
    })
  }
...
export const searchGlobal: AgentTool = {
  ...
  execute: async (
    ...
    try {
      ...
      return toolError(errMsg, { result: `Search error: ${errMsg}` })
    } finally {
      execSpan?.end()
    }
  },
...
export const getSlackRelatedMessages: AgentTool = {
  ...
  execute: async (
    ...
    if (!email) {
      ...
      execSpan?.setAttribute("error", errorMsg)
      return toolError("Missing user email", { result: errorMsg })
    }
    ...
    try {
      ...
      if (!items.length) {
        ...
        return toolSuccess(noResultsMsg, {
          contexts: [],
          metadata: {
  ...
  execute: async (
    ...
    if (agentPrompt) {
      ...
      if (
        ...
        return toolError("Slack not enabled", {
          result:
    ...
    if (!params.user_email) {
      ...
      execSpan?.setAttribute("error", errorMsg)
      return toolError("Missing target_user_email parameter", {
        result: errorMsg,
      })
    }
    ...
    try {
      ...
      if (children.length === 0) {
        return toolSuccess(
          `No Slack profile found for email: ${params.user_email}`,
          {
            contexts: [],
            metadata: {
              targetUser: params.user_email,
            },
          },
        )
      }
      ...
      if (!userProfileDoc.fields) {
        ...
        return toolError("Invalid Slack user profile document", {
      ...
      return toolSuccess(
        `Successfully retrieved Slack profile for ${params.user_email}.`,
        {
          contexts: [userFragment],
          metadata: {
            targetUser: params.user_email,
  ...
  execute: async (
    ...
    try {
      ...
      if (
        ...
        return toolError("No reasoning generated", {
...
export const searchGmail: AgentTool = {
  ...
  execute: async (
    ...
    try {
      ...
      const errMsg = getErrorMessage(error)
      execSpan?.setAttribute("error", errMsg)
      return toolError(errMsg, { result: `Gmail search error: ${errMsg}` })
    } finally {
      execSpan?.end()
    }
  },
...
export const searchDriveFiles: AgentTool = {
  ...
  execute: async (
    ...
    try {
      ...
      const errMsg = getErrorMessage(error)
      execSpan?.setAttribute("error", errMsg)
      return toolError(errMsg, { result: `Drive files search error: ${errMsg}` })
    } finally {
      execSpan?.end()
    }
  },
