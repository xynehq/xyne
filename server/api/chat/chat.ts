...
import { getModelConfiguration, getModelValueFromLabel } from "@/ai/modelConfig"
...
import { AgenticRAG } from "./pi-mono/agentic"
...
function deletePiMonoSessionFile(chatId: string): void {
  const sessionFilePath = join(config.piMonoSessionsDir, `${chatId}.jsonl`)
  if (existsSync(sessionFilePath)) {
    try {
      ...
      Logger.error(
        error,
        `Failed to delete pi-mono session file for chat ${chatId}`,
      )
    }
  }
}
...
export const ChatDeleteApi = async (c: Context) => {
  ...
  try {
    ...
    deletePiMonoSessionFile(chatId)
        ...
        try {
          ...
          if (
            ...
            if (deepResearchEnabled) {
              ...
              reasoning =
                userRequestsReasoningAndEnabled &&
                modelSupportsReasoning(config.defaultDeepResearchModel)
              ...
              reasoning =
                userRequestsReasoningAndEnabled &&
                modelSupportsReasoning(config.defaultWebSearchModel)
