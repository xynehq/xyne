     ...
     async (toolCallId, params, signal, onUpdate, ctx: XyneToolContext) => {
       ...
       try {
         ...
         const filteredAgents = params.query
           ? agents.filter((agent) => 
               agent.agentName.toLowerCase().includes(params.query!.toLowerCase()) ||
               (agent.description?.toLowerCase().includes(params.query!.toLowerCase()) ?? false)
             )
           : agents
         ...
         return {
           content: [{ type: "text", text: `Failed to list custom agents: ${errMsg}` }],
           isError: true,
