/**
   ...
   const participantsSchema = Type.Object({
     from: Type.Optional(Type.Array(Type.String({ 
       description: "Sender identifier string. Email is preferred; full name or organization name can also work." 
     }))),
     to: Type.Optional(Type.Array(Type.String({ 
       description: "Primary recipient identifier string. Email is preferred; full name or organization name can also work." 
     }))),
     cc: Type.Optional(Type.Array(Type.String({ 
       description: "CC recipient identifier string. Email is preferred; full name or organization name can also work." 
     }))),
     bcc: Type.Optional(Type.Array(Type.String({ 
       description: "BCC recipient identifier string. Email is preferred; full name or organization name can also work." 
     }))),
   }, { 
     description: "Structured Gmail participant filter object with optional `from`, `to`, `cc`, and `bcc` string arrays." 
   })
...
`
   ...
   const searchGmailParams = Type.Object({
     ...
     labels: Type.Optional(Type.Array(Type.String({ description: "Gmail label" }), { 
       description: "Optional Gmail label strings used to narrow the search. Common values include `IMPORTANT`, `STARRED`, `UNREAD`, `CATEGORY_PERSONAL`, `CATEGORY_SOCIAL`, `CATEGORY_PROMOTIONS`, `CATEGORY_UPDATES`, `CATEGORY_FORUMS`, `DRAFT`, `SENT`, `INBOX`, `SPAM`, and `TRASH`."
     })),
     timeRange: Type.Optional(Type.Object({
       startTime: Type.Optional(Type.String()),
       endTime: Type.Optional(Type.String())
     })),
     participants: Type.Optional(Type.Composite([participantsSchema], {
       description: "Advanced email communication filtering with intelligent resolution of names, organizations, and email addresses. Supports complex multi-participant email queries with automatic name-to-email mapping. - Structure: {from?: string[], to?: string[], cc?: string[], bcc?: string[]}. - Each field accepts arrays containing email addresses, full names, first names, or organization names."
     }))
   })
       ...
       try {
         ...
         if (agentAppEnums && agentAppEnums.length > 0) {
           if (!agentAppEnums.includes(Apps.Gmail)) {
             return {
               content: [{ type: "text", text: "Gmail is not allowed for this agent. Cannot search." }],
               isError: true,
         ...
         if (!email) {
           return {
             content: [{ type: "text", text: "Email is required for Gmail search." }],
             isError: true,
         ...
         return {
           content: [{ type: "text", text: `Found ${fragments.length} Gmail messages` }],
