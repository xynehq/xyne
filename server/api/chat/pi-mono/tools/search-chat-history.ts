/**
   ...
   const searchChatHistoryParams = Type.Object({
     query: Type.String({
       description: "Search query to find relevant earlier messages in the conversation",
       minLength: 1,
     }),
     chatId: Type.Optional(Type.String({
       description: "Conversation to search. Use the chatId from 'Relevant Past Experiences' when searching a past conversation; omit to search the current conversation only.",
     })),
   ...
   export const searchChatHistoryTool = createXyneTool(
     ...
     async (toolCallId, params, signal, onUpdate, ctx: XyneToolContext) => {
       ...
       try {
         ...
         if (!email) {
           return {
             content: [{ type: "text", text: "User email is required to search chat history." }],
             isError: true,
         ...
         return {
           content: [{ type: "text", text: `Chat history search error: ${errMsg}` }],
           isError: true,
