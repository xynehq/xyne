    ...
    try {
      ...
      if (params.filters?.targets && params.filters.targets.length > 0) {
        targets = params.filters.targets.map(t => {
          switch (t.type) {
            case "collection":
              return { type: "collection" as const, collectionId: t.collectionId! }
            case "folder":
              return { type: "folder" as const, folderId: t.folderId! }
            case "file":
              return { type: "file" as const, fileId: t.fileId! }
            case "path":
              return { type: "path" as const, collectionId: t.collectionId!, path: t.path! }
          }
        })
      }
      ...
      if (result.error) {
        return {
          content: [{ type: "text", text: result.error.message || "KB search failed" }],
          isError: true,
      ...
      return {
        content: [{ type: "text", text: `Found ${fragments.length} KB results` }],
        details: { fragments, query: params.query, toolName: "searchKnowledgeBase" }
      }
    } catch (error) {
