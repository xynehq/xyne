...
const KnowledgeBaseTargetSchema = Type.Union([
  ...
  }, { description: "Object shape: `{ type: \"collection\", collectionId: string }`. Targets an entire collection root. Best when the user names a known collection or you want to browse/search everything inside it." }),
  Type.Object({
  ...
  }, { description: "Object shape: `{ type: \"folder\", folderId: string }`. Targets a folder subtree inside a collection. Useful after `ls` returns a folder ID or the folder is already known." }),
  Type.Object({
  ...
  }, { description: "Object shape: `{ type: \"file\", fileId: string }`. Targets one exact file. Use for pinpointed browsing/search when the relevant document is already known." }),
  Type.Object({
  ...
  }, { description: "Object shape: `{ type: \"path\", collectionId: string, path: string }`. Targets a collection-relative path when the location is known or easier to express than raw folder/file IDs." }),
...
const lsKnowledgeBaseParams = Type.Object({
  target: Type.Optional(Type.Object({
    ...
    collectionId: Type.Optional(Type.String({ description: "Collection ID (required for type: collection or path)" })),
    folderId: Type.Optional(Type.String({ description: "Folder ID (required for type: folder)" })),
    fileId: Type.Optional(Type.String({ description: "File ID (required for type: file)" })),
    path: Type.Optional(Type.String({ description: "Path (required for type: path)" })),
  }, {
    description:
  ...
  depth: Type.Optional(Type.Number({ 
    ...
    maximum: 5, 
    default: 1,
    description: "Traversal depth from the target. `1` lists immediate children only. Start shallow and increase depth only when the task truly needs more hierarchy.",
  })),
  limit: Type.Optional(Type.Number({ 
    minimum: 1, 
    maximum: 100,
    description: "Maximum number of browse rows to return from the flattened listing. Keep this small for discovery and page with `offset` when needed.",
  })),
  ...
  metadata: Type.Optional(Type.Boolean({ 
    default: false,
    description: "Return persisted row metadata when true. Leave false for normal navigation; enable when you need details like description, mime type for filtering PDFs or other file types, timestamps, or collection metadata.",
  })),
})
  ...
  async (toolCallId, params, signal, onUpdate, ctx: XyneToolContext) => {
    ...
    try {
      ...
      if (params.target) {
        ...
        switch (t.type) {
          case "collection":
            target = { type: "collection" as const, collectionId: t.collectionId! }
            break
          case "folder":
          ...
          case "file":
            ...
            break
          case "path":
            target = { type: "path" as const, collectionId: t.collectionId!, path: t.path! }
            break
        }
      }
      ...
      if (result.error) {
        return {
          content: [{ type: "text", text: result.error.message || "KB ls failed" }],
          isError: true,
