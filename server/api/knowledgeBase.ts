...
export const CreateFolderApi = async (c: Context) => {
  ...
  try {
    ...
    loggerWithChild({ email: userEmail }).info(
      `Created folder: ${folder.id} in KB: ${kbId}`
    );
...
async function ensureFolderPath(
  ...
  return ensureFolderPath(db, kbId, pathParts.slice(1), currentFolderId, folderCache);
}
  ...
  try {
    ...
    if (isComplete) {
      ...
      const summary = {
        ...
        skipped: results.filter((r: any) => !r.success && r.message.includes('Skipped')).length,
        failed: results.filter((r: any) => !r.success && !r.message.includes('Skipped')).length,
    ...
    const duplicateStrategy = (formData.get("duplicateStrategy") as DuplicateStrategy) || DuplicateStrategy.RENAME;
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      // Skip system files
      if (file.name === '.DS_Store' || file.name.startsWith('._') || file.name === 'Thumbs.db' || file.name === 'desktop.ini') {
        uploadResults.push({
          success: false,
          fileName: file.name,
      ...
      try {
        ...
        if (originalFileName === '.DS_Store' || originalFileName.startsWith('._') || originalFileName === 'Thumbs.db' || originalFileName === 'desktop.ini') {
          uploadResults.push({
            success: false,
            fileName: originalFileName,
            parentId: targetParentId,
        ...
        if (pathParts.length > 0) {
          targetParentId = await ensureFolderPath(db, kbId, pathParts, parentId, folderCache);
        }
        ...
        const existingFile = await getKbItemByPath(db, kbId, targetPath, fileName);
          ...
          if (existingKbFile && existingKbFile.checksum === checksum) {
            ...
            if (duplicateStrategy === DuplicateStrategy.SKIP) {
              uploadResults.push({
                success: false,
                fileName: originalFileName,
                message: "Skipped: File with same name exists (different content)",
                duplicateId: existingFile.id,
                parentId: targetParentId,
              });
              continue;
        ...
        const { item, file: kbFile } = await db.transaction(async (tx) => {
          return await createFileItem(
            ...
            checksum,
            {
              ...
              wasOverwritten: existingFile && duplicateStrategy === DuplicateStrategy.OVERWRITE,
            },
            user.id,
            user.email
          );
        ...
        try {
          if (baseMimeType === "application/pdf") {
            // Process PDF
            const result = await extractTextAndImagesWithChunksFromPDF(storagePath, vespaDocId, false);
            chunks = result.text_chunks;
            chunks_pos = result.text_chunk_pos;
          } else if (isDocxFile(baseMimeType)) {
            // Process DOCX
            const result = await extractTextAndImagesWithChunksFromDocx(storagePath, vespaDocId, false);
            chunks = result.text_chunks;
            chunks_pos = result.text_chunk_pos;
          } else if (isPptxFile(baseMimeType)) {
            // Process PPTX
            const result = await extractTextAndImagesWithChunksFromPptx(storagePath, vespaDocId, false);
            chunks = result.text_chunks;
            chunks_pos = result.text_chunk_pos;
            ...
            for (const sheetName of workbook.SheetNames) {
              ...
              for (const row of validRows) {
                const textualCells = row
                  .filter(cell => cell && isNaN(Number(cell)) && cell.toString().trim().length > 0)
            ...
            try {
              ...
              chunks = [`File: ${file.name}, Type: ${baseMimeType}, Size: ${file.size} bytes`];
              chunks_pos = [0];
            }
          }
        } catch (error) {
    ...
    const summary = {
      ...
      skipped: uploadResults.filter(r => !r.success && r.message.includes('Skipped')).length,
      failed: uploadResults.filter(r => !r.success && !r.message.includes('Skipped')).length,
...
export const DeleteItemApi = async (c: Context) => {
  ...
  try {
    ...
    if (!belongsToKb) {
      throw new HTTPException(404, { message: "Item not found in this knowledge base" });
    }
    ...
    if (item.type === "file") {
      ...
      for (const descendantItem of descendants) {
        if (descendantItem.type === "file") {
          ...
          itemsToDelete.push({ item: descendantItem, kbFile: kbFile || undefined });
        } else {
