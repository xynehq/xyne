...
export const ExecuteAgentForWorkflow = async (
  ...
  try {
    ...
    const actualModel =
      agent.model === "Auto" ? defaultBestModel : (agent.model as Models)
...
async function* createStreamingWithDBSave(
  ...
  try {
    ...
    for await (const chunk of originalIterator) {
      ...
      Logger.info(
        `🌊 createStreamingWithDBSave: Forwarded chunk to client: ${chunk.text}`,
      )
...
export const executeAgentForWorkflowWithRag = async (
  ...
  try {
    ...
    Logger.info(
      `[agentCore] Resolved user ID: ${user.id}, workspace ID: ${workspace.id}`,
    )
    ...
    if (!agent) {
      Logger.error(
        `[agentCore] Agent ${params.agentId} not found or permission denied`,
      )
      return {
        success: false,
    ...
    Logger.info(
      `[agentCore] Agent features: RAG=${agent.isRagOn}, Integrations=${agent.appIntegrations?.length || 0}`,
    )
    ...
    Logger.info(
      `[agentCore] User timezone: ${userTimezone}, Date for AI: ${dateForAI}`,
    )
    ...
    Logger.info(
      `[agentCore]    - Workflow PDF attachments: ${params.nonImageAttachmentFileIds?.length || 0}`,
    )
    Logger.info(
      `[agentCore]    - Workflow image attachments: ${params.attachmentFileIds?.length || 0}`,
    )
    Logger.info(`[agentCore]    - Total unique files: ${uniqueFileIds.length}`)

    // Determine if we should use RAG
    const hasWorkflowAttachments =
      (params.attachmentFileIds && params.attachmentFileIds.length > 0) ||
      (params.nonImageAttachmentFileIds &&
        params.nonImageAttachmentFileIds.length > 0)
    const shouldUseRAG =
      (agent.isRagOn && uniqueFileIds.length > 0) || hasWorkflowAttachments // RAG enabled and files available OR workflow has attachments
    ...
    if (shouldUseRAG) {
      ...
      Logger.info(
        `[agentCore] 📊 Classification result: ${JSON.stringify(classification)}`,
      )
      ...
      if (hasAttachments) {
        Logger.info(
          `[agentCore] 🔗 Using UnderstandMessageAndAnswerForGivenContext for ${params.attachmentFileIds?.length || 0} image attachments and ${params.nonImageAttachmentFileIds?.length || 0} document attachments`,
        )
        ...
        if (
          ...
          Logger.info(
            `[agentCore] 📎 PDF attachment IDs:`,
            params.nonImageAttachmentFileIds.map((id) => ({
              id: id,
              type: typeof id,
        ...
        if (
          ...
          const missingFromFileIds = params.nonImageAttachmentFileIds.filter(
            (id) => !uniqueFileIds.includes(id),
          )
          if (missingFromFileIds.length > 0) {
            Logger.error(
              `[agentCore] ❌ CRITICAL: Document attachment IDs missing from uniqueFileIds!`,
              {
            ...
            Logger.info(
              `[agentCore] ✅ All document attachment IDs found in uniqueFileIds`,
            )
          }
        }
        ...
        Logger.info(
          `[agentCore] 🔍 UnderstandMessageAndAnswerForGivenContext parameters:`,
          {
        ...
        Logger.info(
          `[agentCore] 🔗 Using UnderstandMessageAndAnswer for app_integrations (no attachments)`,
        )
      ...
      for await (const chunk of iterator) {
        ...
        if (chunk.metadata?.usage) {
          // Token usage
          totalTokens +=
            chunk.metadata.usage.inputTokens + chunk.metadata.usage.outputTokens
        }
      }
      ...
      if (
        ...
        Logger.error(
          `[agentCore] ❌ CRITICAL: Expected ${params.nonImageAttachmentFileIds.length} document(s) but got 0 citations!`,
          {
            expectedDocuments: params.nonImageAttachmentFileIds,
            finalAnswerContainsError:
              finalAnswer.includes("no content") ||
              finalAnswer.includes("not provided") ||
              finalAnswer.includes("unable to"),
      ...
      Logger.info(
        `[agentCore] 📊 Answer length: ${finalAnswer.length} characters`,
      )
      ...
      if (result.success && result.type === "non-streaming") {
        ...
        Logger.info(
          `[agentCore] Extracted AI response: "${finalAnswer.substring(0, 200)}..." (length: ${finalAnswer.length})`,
        )
        ...
        if (!finalAnswer) {
          Logger.error(
            `[agentCore] AI response is empty! Response object:`,
            result.response,
          )
        ...
        Logger.warn(
          `[agentCore] Unexpected streaming result in non-streaming mode`,
        )
        finalAnswer =
          "AI response was in streaming format but expected non-streaming"
      } else {
        Logger.error(
          `[agentCore] AI execution failed: ${JSON.stringify(result)}`,
        )
        finalAnswer = `AI execution failed: ${result.error || "Unknown error"}`
      }

      Logger.info(
        `[agentCore] Direct LLM call result: ${JSON.stringify(result).substring(0, 700)}...`,
      )
      ...
      if (result.success && result.type === "non-streaming") {
        ...
        totalTokens =
          (result.response?.metadata?.usage?.inputTokens || 0) +
          (result.response?.metadata?.usage?.outputTokens || 0)
        Logger.info(
          `[agentCore] ✅ Successfully extracted response text (${finalAnswer.length} chars)`,
        )
        ...
        Logger.warn(
          `[agentCore] Agent execution returned a streaming response unexpectedly.`,
        )
...
export async function hasUnauthorizedAgent(
  ...
  try {
    Logger.info(
      `Checking unauthorized agents for workflow ${workflowTemplateId}`,
    )

    // 1. Get all step templates for this workflow
    const stepTemplates = await getWorkflowStepTemplatesByTemplateId(
      db,
      workflowTemplateId,
    )
    ...
    const allToolIds = stepTemplates.flatMap(
    ...
    if (aiAgentTools.length === 0) {
      Logger.info(
        `No existing AI agent tools found in workflow ${workflowTemplateId}`,
      )
      return { hasUnauthorized: false, unauthorizedAgents: [] }
    }
    ...
    for (const tool of aiAgentTools) {
      ...
      try {
        ...
        const agentRecord = await getAgentByExternalId(
          db,
          agentExternalId,
          workspaceId,
        )
        ...
        if (!agentRecord) {
          Logger.warn(
            `Agent ${agentExternalId} not found in workspace ${workspaceId}`,
          )
          continue
        }
        ...
        if (agentRecord.isPublic) {
          Logger.info(
            `Agent ${agentExternalId} is public, skipping authorization check`,
          )
          continue
        }
        ...
        const normalizedUpdatedEmails = updatedUserEmails.map((email) =>
          email.toLowerCase(),
        )
    ...
    if (hasUnauthorized) {
      ...
      Logger.info(
        `All agents in workflow ${workflowTemplateId} are properly authorized`,
      )
    }
    ...
    Logger.error(
      error,
      `Failed to check unauthorized agents for workflow ${workflowTemplateId}`,
    )
