...
export const SdkTokenMiddleware = async (c: Context, next: Next) => {
  ...
  try {
    ...
    if (!workspaceId || decoded.token_type !== "sdk") {
      throw new HTTPException(401, { message: "Invalid token" })
    }

    // 2. Look up sdkConfig → get tokenSecret
    const config = await getSdkConfigByWorkspaceExternalId(db, workspaceId)
    ...
    c.set("sdkSub", payload.sub as string)
    c.set("sdkEmail", payload.email as string | undefined)

    await next()
  } catch (error) {
    if (error instanceof HTTPException) throw error
    Logger.error(error, "SDK token validation failed")
    throw new HTTPException(401, { message: "Invalid or expired token" })
  }
}

export const SdkCorsMiddleware = async (c: Context, next: Next) => {
