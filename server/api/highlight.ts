...
class OrderedWindowFinder {
  public findMinimumWindow(
    ...
    for (let sentenceId = 0; sentenceId < tokenPositions.length; sentenceId++) {
      if (tokenPositions[sentenceId].length > 0) {
        // Only include tokens that have occurrences
        for (const pos of tokenPositions[sentenceId])
          merged.push({ pos, sentenceId })
      }
    }
    ...
    const result =
      bestL === null || bestR === null ? null : { start: bestL, end: bestR }
    return result
  }
}
...
const mergeCloseMatches = (
  ...
  for (let i = 1; i < sortedMatches.length; i++) {
    ...
    if (gap <= 128) {
      ...
      currentMatch.endIndex = Math.max(
...
export const HighlightApi = async (c: Context) => {
  try {
    ...
    const lowerCaseDoc = caseSensitive
      ? documentContent
      : documentContent.toLowerCase()
    const lowerCaseChunk = caseSensitive ? chunkText : chunkText.toLowerCase()
    const { norm: normalizedDoc, map: normalizedMap } =
      normalizeTextWithMap(lowerCaseDoc)
    ...
    for (let i = 0; i < queryTokens.length; i++) {
      ...
      if (windowPositions.length > 0) {
        ...
        const lastCharOrig =
          normalizedMap[lastCharIndex] ?? startOrig + token.length - 1
