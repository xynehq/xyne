import {
  ...
  type EmailFilters,
} from "../integrations/google/gmail-fetcher"
...
const fetchGmailForUser = async (
  ...
  try {
    ...
    if (!connector.credentials) {
      Logger.error(
        `Connector for user: ${userEmail} does not have a service account.`,
      )
      process.exit(1)
    }
    ...
    if (fetchedData.length > 0) {
      Logger.info(
        `Successfully fetched data for ${fetchedData.length} email(s).`,
      )
      ...
      Logger.info(
        `Email Body (first 200 chars): ${firstEmail.emailBody.substring(0, 200)}...`,
      )
      Logger.info("-------------------------------------\n")
    } else {
