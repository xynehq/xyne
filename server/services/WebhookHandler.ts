...
export class WebhookHandler {
      ...
      for (const tool of webhookTools) {
        try {
          ...
          if (webhookPath && typeof webhookPath === "string") {
            ...
            const cleanPath = webhookPath.startsWith("/")
              ? webhookPath
              : `/${webhookPath}`
            ...
            Logger.info(
              `📝 Loaded webhook: ${cleanPath} -> Template: ${tool.templateId}, Tool: ${tool.id}`,
            )
          }
        } catch (error) {
  ...
  async executeWorkflowFromWebhook(
    ...
    try {
      ...
      Logger.info(
        `📋 Found ${allStepTemplates.length} step templates in workflow:`,
        allStepTemplates.map((s) => ({
          id: s.id,
          name: s.name,
          type: s.type,
      ...
      for (const stepTemplate of allStepTemplates) {
        ...
        const isWebhookStep =
          stepTemplate.toolIds &&
          stepTemplate.toolIds.length > 0 &&
        ...
        Logger.info(
          `📝 Created step execution: ${stepExecution.name} (${stepExecution.id}) - Status: ${stepExecution.status}, IsWebhook: ${isWebhookStep}`,
        )

        // Create tool executions for this step (with webhook completion logic)
        if (stepTemplate.toolIds && stepTemplate.toolIds.length > 0) {
          await this.createToolExecutions(
            stepExecution.id,
            stepTemplate.toolIds,
            webhookData,
          )
      ...
      const tools = await this.getWorkflowTools(
        templateId,
        template[0].userId,
        template[0].workspaceId,
      )
      ...
      const rootStepExecution = stepExecutions.find(
        (se) => se.workflowStepTemplateId === rootStepId,
      )

      if (!rootStepExecution) {
        throw new Error(
          `Root step execution not found for template ${templateId}`,
        )
      }

      Logger.info(
        `🏁 Starting workflow execution from root step: ${rootStepExecution.name}`,
      )

      // Execute the workflow chain with empty previous results (webhook is first step)
      executeWorkflowChain(execution.id, rootStepExecution.id, tools, {})
        .then(async () => {
          // After workflow chain completes, do a final completion check
          Logger.info(
            `🔄 Workflow chain completed for ${execution.id}, checking final completion status`,
          )
          try {
      ...
      setTimeout(async () => {
        try {
          Logger.info(
            `🔄 Running delayed completion check for workflow ${execution.id}`,
          )
          const workflowModule = await import("../api/workflow")
          await workflowModule.checkAndCompleteWorkflow(execution.id)
        } catch (error) {
  ...
  async handleWebhookRequest(c: Context): Promise<Response> {
    ...
    const expectedMethod =
      webhook.config?.httpMethod || webhook.value?.httpMethod || "POST"
    if (c.req.method !== expectedMethod) {
      Logger.warn(
        `Method ${c.req.method} not allowed for ${path}, expected ${expectedMethod}`,
      )
      return c.json(
        {
    ...
    Logger.info(
      `📥 Webhook triggered: ${path} (Template: ${webhook.workflowTemplateId})`,
    )

    // Validate request body for POST method
    if (expectedMethod === "POST") {
      const expectedRequestBody =
        webhook.config?.requestBody || webhook.value?.requestBody
      if (expectedRequestBody && typeof expectedRequestBody === "string") {
        try {
          ...
          if (!expectedBodyObj || typeof expectedBodyObj !== "object") {
            Logger.warn(
              `Invalid JSON structure in configured request body for webhook ${path}`,
            )
            return c.json(
              {
          ...
          try {
            ...
            if (contentType.includes("application/json")) {
              ...
              Logger.warn(
                `Expected JSON content type for POST webhook ${path}, got: ${contentType}`,
              )
              return c.json(
                {
                  error:
                    "Content-Type must be application/json for POST webhooks with request body validation",
                },
                400,
              )
            }
            ...
            Logger.warn(
              `Failed to parse JSON body for webhook ${path}: ${parseError}`,
            )
            return c.json(
              {
          ...
          if (!incomingBody || typeof incomingBody !== "object") {
            ...
            return c.json(
              {
                error:
                  "Request body must be a valid JSON object matching the configured structure",
              },
              400,
            )
          }
          ...
          if (missingKeys.length > 0) {
            Logger.warn(
              `Missing required fields in request body for webhook ${path}: ${missingKeys.join(", ")}`,
            )
            return c.json(
              {
          ...
          Logger.error(
            `Invalid JSON in configured request body for webhook ${path}: ${jsonError}`,
          )
          return c.json(
            {
              error:
                "Invalid JSON configuration in webhook request body template",
            },
            500,
          )
        }
        ...
        Logger.warn(
          `Invalid request body type for webhook ${path}: expected string, got ${typeof expectedRequestBody}`,
        )
        return c.json(
          {
        ...
        Logger.warn(
          `POST webhook ${path} missing required request body configuration`,
        )
        return c.json(
          {
    ...
    if (Object.keys(expectedHeaders).length > 0) {
      ...
      for (const [headerName, expectedValue] of Object.entries(
        ...
        const actualValue =
          requestHeaders[headerName.toLowerCase()] || requestHeaders[headerName]

        if (!actualValue) {
          missingHeaders.push(headerName)
        } else if (expectedValue && actualValue !== expectedValue) {
          Logger.warn(
            `Header ${headerName} value mismatch for webhook ${path}: expected '${expectedValue}', got '${actualValue}'`,
          )
          return c.json(
            {
      ...
      if (missingHeaders.length > 0) {
        Logger.warn(
          `Missing required headers for webhook ${path}: ${missingHeaders.join(", ")}`,
        )
        return c.json(
          {
    ...
    try {
      if (c.req.method !== "GET") {
        ...
        if (contentLength && parseInt(contentLength) > 10 * 1024 * 1024) {
          Logger.warn(
            `Request body too large for webhook ${path}: ${contentLength} bytes`,
          )
    ...
    try {
      ...
      Logger.info(
        `✅ Workflow execution started: ${executionId} for template: ${webhook.workflowTemplateId}`,
      )
    ...
    switch (authType) {
      case "basic":
        if (!authHeader || !authHeader.startsWith("Basic ")) {
          Logger.warn(
            "Basic authentication required but no Authorization header provided",
          )
          return false
        }
        ...
        const basicCredentials = Array.isArray(webhook.config?.credentials)
          ? webhook.config.credentials
          : []
        const basicSelectedCredential = basicCredentials.find(
          (cred: unknown) =>
            typeof cred === "object" &&
            cred !== null &&
            "isSelected" in cred &&
            (cred as Record<string, unknown>).isSelected === true,
      ...
      case "bearer":
        if (!authHeader || !authHeader.startsWith("Bearer ")) {
          Logger.warn(
            "Bearer authentication required but no Authorization header provided",
          )
          return false
        }
        ...
        // Find the selected credential from credentials array
        const bearerCredentials = Array.isArray(webhook.config?.credentials)
          ? webhook.config.credentials
          : []
        const bearerSelectedCredential = bearerCredentials.find(
          (cred: unknown) =>
            typeof cred === "object" &&
            cred !== null &&
            "isSelected" in cred &&
      ...
      case "api_key":
        ...
        if (!apiKey) {
          Logger.warn(
            "API key authentication required but no X-API-Key header or api_key query parameter provided",
          )
          return false
        }

        // Find the selected credential from credentials array
        const apiKeyCredentials = Array.isArray(webhook.config?.credentials)
          ? webhook.config.credentials
          : []
        const apiKeySelectedCredential = apiKeyCredentials.find(
          (cred: unknown) =>
            typeof cred === "object" &&
            cred !== null &&
            "isSelected" in cred &&
  ...
  listWebhooks(): { success: boolean; webhooks: any[]; count: number } {
    try {
      const webhooks = Array.from(webhookRegistry.entries()).map(
        ([path, data]) => ({
          path,
  ...
  private async createToolExecutions(
    ...
    for (const toolId of toolIds) {
      try {
        ...
        if (tool) {
          ...
          Logger.info(
            `📝 Created tool execution for ${tool.type} tool (${toolId}) - Status: ${isWebhookTool ? "COMPLETED" : "PENDING"}`,
          )
        }
      } catch (error) {
        Logger.error(
          `Failed to create tool execution for tool ${toolId}: ${error}`,
        )
      }
    }
  }
  ...
  private async getWorkflowTools(
    ...
    try {
      ...
      Logger.info(
        `📋 Found ${steps.length} step templates:`,
        steps.map((s) => ({
          id: s.id,
          name: s.name,
          toolIds: s.toolIds,
      ...
      Logger.info(
        `🔧 Searching for tools with workspace ${workspaceId} and user ${userId}`,
      )
      ...
      Logger.info(
        `🔨 Retrieved ${tools.length} tools for template ${templateId}:`,
        tools.map((t) => ({
          id: t.id,
          type: t.type,
      ...
      if (tools.length === 0) {
        Logger.warn(
          `⚠️ No tools found for tool IDs: ${allToolIds.join(", ")} in workspace ${workspaceId} for user ${userId}`,
        )
        Logger.info(
          `🔄 Trying to fetch tools without workspace/user restriction for webhook execution...`,
        )
        ...
        Logger.info(
          `🔍 Found ${allMatchingTools.length} matching tools without restriction:`,
          allMatchingTools.map((t) => ({
            id: t.id,
            type: t.type,
            workspaceId: t.workspaceId,
            userId: t.userId,
        ...
        if (allMatchingTools.length > 0) {
          Logger.info(
            `✅ Using tools without workspace/user restriction for webhook execution`,
          )
      ...
      Logger.error(
        `Failed to get workflow tools for template ${templateId}: ${error}`,
      )
      return []
    }
  }
...
${this.generateCurlCommand({
  ...
  url:
    (requestData.url as string) ||
    `http://localhost:3000${requestData.path || "/workflow/webhook"}`,
