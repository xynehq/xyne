...
export class WebhookAuthService {
  ...
  private async validateBasicAuth(
    ...
    try {
      ...
      if (config.selectedCredential) {
        return await this.validateStoredCredential(config.selectedCredential, {
          username,
          password,
        })
      }
  ...
  private async validateBearerAuth(
    c: Context,
    config: WebhookAuthConfig,
  ): Promise<boolean> {
    const authHeader = c.req.header("Authorization")
  ...
  private async validateApiKeyAuth(
    c: Context,
    config: WebhookAuthConfig,
  ): Promise<boolean> {
    const apiKey =
    ...
    if (!apiKey) {
      throw new HTTPException(401, {
        message:
          "API key required in X-API-Key header or api_key query parameter",
      })
    }
  ...
  private async validateStoredCredential(
    ...
    try {
      ...
      Logger.info(
        `Validating stored credential ${credentialId} with provided auth`,
      )
