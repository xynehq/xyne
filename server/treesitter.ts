...
class RustCodeExtractor {
  ...
  private getNodeText(sourceCode: string, node: SyntaxNode): string { // Use imported SyntaxNode
    return sourceCode.substring(node.startIndex, node.endIndex);
  }

  // Traverse the node tree and collect nodes of specific types
  private traverseTree(node: SyntaxNode, callback: (node: SyntaxNode) => void): void { // Use imported SyntaxNode
    callback(node);
  ...
  private extractSymbolNames(sourceCode: string, rootNode: SyntaxNode): string[] { // Use imported SyntaxNode
  ...
  private extractCodeChunksFlattened(sourceCode: string, rootNode: SyntaxNode): { // Use SyntaxNode type
    kinds: string[],
    names: string[],
    contents: string[],
    startLines: number[],
    ...
    const itemTypes = [ // Keep only one declaration
      'function_item',
      'struct_item',
      'enum_item',
      'trait_item',
  ...
  private extractDocCommentsFlattened(sourceCode: string, rootNode: SyntaxNode): { // Use SyntaxNode type
     texts: string[],
    ...
    const lineNumbers = Object.keys(docLines).map(Number).sort((a, b) => a - b);
  ...
  private extractDependenciesFlattened(sourceCode: string, rootNode: SyntaxNode): { // Use SyntaxNode type
     names: string[],
     fullPaths: string[],
     lines: number[]
   } {
    ...
    this.traverseTree(rootNode, (node: SyntaxNode) => { // Add type annotation
  ...
  private extractFeatureFlags(sourceCode: string, rootNode: SyntaxNode): string[] { // Use imported SyntaxNode
  ...
  public async extract(filePath: string): Promise<any> {
    try {
      ...
      const { kinds: codeChunkKinds, names: codeChunkNames, contents: codeChunkContents, startLines: codeChunkStartLines, endLines: codeChunkEndLines } = this.extractCodeChunksFlattened(sourceCode, rootNode);
      const { texts: docCommentsTexts, targets: docCommentsTargets, startLines: docCommentsStartLines } = this.extractDocCommentsFlattened(sourceCode, rootNode);
      const { names: dependenciesNames, fullPaths: dependenciesFullPaths, lines: dependenciesLines } = this.extractDependenciesFlattened(sourceCode, rootNode);
...
async function processRustFile(filePath: string) {
  try {
    ...
    console.log(`Successfully inserted document with ID ${extractedData.docId} for ${filePath}.`);
    ...
    console.log(`- Extracted ${extractedData.code_chunk_names.length} code chunks`); // Use names length as chunk count indicator
    console.log(`- Extracted ${extractedData.doc_comments_texts.length} doc comments`);
    console.log(`- Extracted ${extractedData.dependencies_names.length} dependencies`);
    console.log(`- Extracted ${extractedData.feature_flags.length} feature flags`);
    // No output path needed anymore
    // console.log(`- Output: ${outputPath}`);

    return {
      status: "success", // Indicate success of extraction and insertion
      stats: { // Update stats object keys
