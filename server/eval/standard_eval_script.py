...
def compute_metrics(qids_to_relevant_passageids, qids_to_ranked_candidate_passages, k=10):
    """Compute MRR, NDCG, Precision, Recall, and F1-score metrics.
    
    Args:    
    qids_to_relevant_passageids (dict): dictionary of query-passage mapping
    qids_to_ranked_candidate_passages (dict): dictionary of query-passage candidates
    
    Returns:
        dict: dictionary of metrics {'MRR': <MRR Score>, 'NDCG': <NDCG Score>, 'Precision': <Precision Score>, 'Recall': <Recall Score>, 'F1': <F1 Score>}
    """
    all_scores = {}
    MRR = 0
    ndcgs = []
    ...
    for qid in qids_to_ranked_candidate_passages:
        if qid in qids_to_relevant_passageids:
            ...
            y_true = [1 if pid in target_pid else 0 for pid in candidate_pid[:k]]
            y_score = [1] * len(y_true)
            ...
            precisions.append(precision_score(y_score, y_true, zero_division=0))
            recalls.append(recall_score(y_score, y_true, zero_division=0))
            f1s.append(f1_score(y_score, y_true, zero_division=0))
    ...
    all_scores['NDCG @10'] = np.mean(ndcgs)
    all_scores['Precision @10'] = np.mean(precisions)
    all_scores['Recall @10'] = np.mean(recalls)
    all_scores['F1 @10'] = np.mean(f1s)
