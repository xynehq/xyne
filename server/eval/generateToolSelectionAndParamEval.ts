...
function evaluateResponse({
  ...
  console.log("Generated tool(s):", output.tool || "none")
  ...
  console.log(
    pc.green(
      `Tool match score: ${(toolMatchScore * 100).toFixed(1)}%, Argument match score: ${(argsScore * 100).toFixed(1)}%, Sequence match score: ${(sequenceScore * 100).toFixed(1)}%`,
    ),
  )
...
async function runEvaluation(userCtx: string) {
 ...
 AVAILABLE_TOOLS:\n\n`
  ...
  for await (const item of data) {
    ...
    let output: { answer?: string; tool?: string | string[]; arguments?: any } =
      {
        answer: null,
        tool: null,
        arguments: null,
