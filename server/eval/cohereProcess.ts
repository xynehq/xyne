...
import { generateEmbedding } from "@/ai/provider/bedrock"

const corpusPath = path.resolve(import.meta.dirname, "data/scifact/corpus.jsonl")
...
//   console.log(allChunks,allChunks.length,'chunks')
  const embeddings = await generateEmbedding(allChunks)
  try {
    let embeddingIndex = 0
  for await (const doc of processedbatch) {
    ...
    doc.fields["chunk_embeddings"] = chunkMap
        ...
        console.log('error in chunks embed', error)
  }
