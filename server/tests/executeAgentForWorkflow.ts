...
async function testExecuteAgent() {
  ...
  try {
    ...
    if (result.type === "streaming") {
      ...
      console.log(
        "🔄 Starting iterator consumption (this will trigger the missing logs)...",
      )
...
async function testExecuteAgentWithMixedAttachments() {
  console.log(
    "🧪 Testing ExecuteAgentForWorkflow with PDF text + embedded image...",
  )

  const result = await ExecuteAgentForWorkflow({
    agentId: "zk3i1cycov8i5arrhk5u7y20",
    userQuery:
      "Analyze the resume content and describe any images or visual elements you see",
    workspaceId: "uxwyx18h74vdch0j8ir46aka",
    userEmail: "aman.asrani@juspay.in",
    isStreamable: false,
    temperature: 0.7,
