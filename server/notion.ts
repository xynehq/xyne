  ...
  for (const page of pages) {
    ...
    for await (const result of crawl(page.id)) {
      ...
      process.stdout.write(
        `${page.id} ${count} ${Math.floor((count / pages.length) * 100)}`,
      );
      if (result.success) {
...
export const initNotion = async () => {
  ...
  if (!data) {
    ...
    data = await Promise.all(
      [...finalData].map(async (doc, i) => ({
        properties: {
          ...doc,
        },
        vectors: (
          await extractor(getVectorStr(doc.title, doc.chunk), {
            pooling: "mean",
            normalize: true,
          })
        ).tolist()[0], // Add the generated vector
      })),
    );
