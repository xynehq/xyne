...
export type ProcessingJob = FileProcessingJob | CollectionProcessingJob | FolderProcessingJob
...
async function handleRetryFailure(
  ...
  if (newRetryCount >= maxRetries) {
    ...
    if (entityType === ProcessingJobType.COLLECTION) {
      await db.update(collections).set(updateData).where(eq(collections.id, entityId))
    } else {
      await db.update(collectionItems).set(updateData).where(eq(collectionItems.id, entityId))
    }
  } else {
...
export async function processJob(job: { data: ProcessingJob }) {
  ...
  switch (jobType) {
    ...
    case ProcessingJobType.COLLECTION:
      return await processCollectionJob(jobData as CollectionProcessingJob, startTime)
...
async function processFileJob(jobData: FileProcessingJob, startTime: number) {
  ...
  try {
    ...
    const vespaFileName = targetPath === "/"
    ...
    await handleRetryFailure(ProcessingJobType.FILE, fileId, file.retryCount || 0, errorMessage)
    
    throw error // Let pg-boss handle retries
  }
}

async function processCollectionJob(jobData: CollectionProcessingJob, startTime: number) {
  ...
  try {
    ...
    const vespaDoc = {
      ...
      metadata: JSON.stringify({
        version: "1.0",
        lastModified: Date.now(),
        ...(typeof col.metadata === 'object' && col.metadata ? col.metadata : {}),
      }),
    ...
    const endTime = Date.now()
    Logger.info(`Successfully processed collection Vespa insertion: ${collectionId} in ${endTime - startTime}ms`)
    
  } catch (error) {
    const errorMessage = getErrorMessage(error)
    Logger.error(error, `Failed to process collection Vespa insertion: ${collectionId} - ${errorMessage}`)
    
    // Use common retry handling function with existing collection data
    await handleRetryFailure(ProcessingJobType.COLLECTION, collectionId, col.retryCount || 0, errorMessage)
    
    throw error // Let pg-boss handle retries
  }
}

async function processFolderJob(jobData: FolderProcessingJob, startTime: number) {
  ...
  if (fol.type !== ProcessingJobType.FOLDER) {
  ...
  try {
    ...
    const endTime = Date.now()
    Logger.info(`Successfully processed folder Vespa insertion: ${folderId} in ${endTime - startTime}ms`)
    
  } catch (error) {
    const errorMessage = getErrorMessage(error)
    Logger.error(error, `Failed to process folder Vespa insertion: ${folderId} - ${errorMessage}`)
    
    // Use common retry handling function with existing folder data
    await handleRetryFailure(ProcessingJobType.FOLDER, folderId, fol.retryCount || 0, errorMessage)
