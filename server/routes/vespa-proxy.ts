...
export const vespaIfDocumentsExistProxy = handleProxy(
  ({ docIds }) => vespaClient.ifDocumentsExist(docIds),
  "Vespa if documents exist proxy error",
)

// Vespa if mail documents exist proxy endpoint (Hono)
export const vespaIfMailDocumentsExistProxy = handleProxy(
  ({ mailIds }) => vespaClient.ifMailDocumentsExist(mailIds),
  "Vespa if mail documents exist proxy error",
)

// Vespa if documents exist in chat container proxy endpoint (Hono)
export const vespaIfDocumentsExistInChatContainerProxy = handleProxy(
  ({ docIds }) => vespaClient.ifDocumentsExistInChatContainer(docIds),
  "Vespa if documents exist in chat container proxy error",
)

// Vespa if documents exist in schema proxy endpoint (Hono)
export const vespaIfDocumentsExistInSchemaProxy = handleProxy(
  ({ schema, docIds }) => vespaClient.ifDocumentsExistInSchema(schema, docIds),
  "Vespa if documents exist in schema proxy error",
)
...
// Vespa if mail doc exist proxy endpoint (Hono)
export const vespaIfMailDocExistProxy = handleProxy(
  ({ docId }, email) => vespaClient.ifMailDocExist(email, docId),
  "Vespa if mail doc exist proxy error",
)
