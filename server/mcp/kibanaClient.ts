...
export default class KibanaClient {
  ...
  private truncateResultsByTokens(
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      if (totalTokens + resultTokens > maxTokens) {
        this.logger.info(`Token limit reached. Truncating results at index ${i}/${results.length}`);
        this.logger.info(`Current tokens: ${totalTokens}, Result tokens: ${resultTokens}, Max: ${maxTokens}`);
  ...
  async searchLogs(payload: KibanaSearchPayload): Promise<KibanaSearchResult> {
    ...
    const missingFields = requiredFields.filter(field => !payload[field as keyof KibanaSearchPayload]);
    ...
    if (!query_terms.length && !or_terms.length && !not_terms.length) {
      throw new Error("At least one of query_terms, or_terms, or not_terms must be provided");
    }
    ...
    if (!["concise", "detailed"].includes(responseFormat)) {
      throw new Error("Invalid response_format. Must be 'concise' or 'detailed'.");
    }

    // Progressive reduction strategy
    const attempts = [maxResults, Math.floor(maxResults / 2), Math.floor(maxResults / 4), Math.floor(maxResults / 8), Math.min(50, maxResults)]
      .filter(a => a > 0);
    ...
    for (let attemptNum = 0; attemptNum < attempts.length; attemptNum++) {
      ...
      try {
        this.logger.info(`Attempt ${attemptNum + 1}: Trying with max_results=${currentMaxResults}`);
        ...
        this.logger.debug(`Query structure: ${JSON.stringify(boolQuery, null, 2)}`);
        ...
        this.logger.info(`Kibana response length: ${JSON.stringify(response.data).length}`);
        ...
        this.logger.info(`Kibana search results (format: ${responseFormat}): ${JSON.stringify(processedResults.slice(0, 3))}...`);

        const { truncatedResults, totalTokens, wasTruncated } = this.truncateResultsByTokens(
          processedResults, 
          maxTokens, 
          responseFormat
        );

        this.logger.info(`Results: ${processedResults.length} -> ${truncatedResults.length} (tokens: ${totalTokens})`);
        ...
        if (!wasTruncated || attemptNum === attempts.length - 1) {
          ...
          this.logger.info("Results were truncated, trying with fewer max_results");
          continue;
        }
        ...
        if (axios.isAxiosError(error)) {
          ...
          this.logger.error(`HTTP Error calling Kibana API: ${statusCode} - ${JSON.stringify(errorContent)}`);
          
          if (attemptNum === attempts.length - 1) {
            throw new Error(`Kibana API Error (${statusCode}): ${JSON.stringify(errorContent)}`);
          } else {
          ...
          if (attemptNum === attempts.length - 1) {
            ...
            this.logger.info(`Retrying with fewer results due to error: ${error.message}`);
            continue;
          }
        }
      }
