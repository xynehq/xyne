...
import { Apps, AuthType, ConnectorStatus, SyncJobStatus, DriveEntity, GooglePeopleEntity } from "@/shared/types";
...
const listUsers = async (admin: admin_directory_v1.Admin, domain: string): Promise<admin_directory_v1.Schema$User[]> => {
    ...
    try {
        do {
            const res: GaxiosResponse<admin_directory_v1.Schema$Users> = await admin.users.list({
                domain: domain,
                maxResults: 500,
                orderBy: "email",
...
export const syncGoogleWorkspace = async (boss: PgBoss, job: PgBoss.Job<any>) => {
    // catch needs access to this
    const syncJobs = await getAppSyncJobs(db, Apps.GoogleWorkspace, AuthType.ServiceAccount)
    try {
        // there should only be 1 job for syncing the google workspace
        if (syncJobs.length > 1 || !syncJobs.length) {
            throw new SyncJobsCountError('Could not sync Google Workspace invalid sync job count')
        }
        ...
        await db.transaction(async (trx) => {
            await updateSyncJob(trx, syncJob.id, { config: { updatedAt: new Date() }, lastRanOn: new Date(), status: SyncJobStatus.Successful })
...
export const handleGoogleOAuthIngestion = async (boss: PgBoss, job: PgBoss.Job<any>) => {
    ...
    try {
        ...
        const connector: SelectConnector = await getOAuthConnectorWithCredentials(db, data.connectorId)
...
const insertFilesForUser = async (googleClient: GoogleClient, userEmail: string, connector: SelectConnector) => {
    try {
        ...
        const documents: VespaFileWithDrivePermission[] = await googleDocsVespa(googleClient, googleDocsMetadata, connector.externalId)
        const driveFiles: VespaFileWithDrivePermission[] = await driveFilesToDoc(rest)

        sendWebsocketMessage('generating embeddings', connector.externalId)
        let allFiles: VespaFileWithDrivePermission[] = [...driveFiles, ...documents].map(v => {
...
export const googleDocsVespa = async (client: GoogleClient, docsMetadata: drive_v3.Schema$File[], connectorId: string): Promise<VespaFileWithDrivePermission[]> => {
...
export const driveFilesToDoc = async (rest: drive_v3.Schema$File[]): Promise<VespaFileWithDrivePermission[]> => {
