...
export async function processTicketJob(
  ...
  try {
    ...
    console.log("\n🔑 ZOHO CLIENT INITIALIZATION: Creating client with stored credentials")
    console.log(`   Connector ID: ${connector.id}`)
    console.log(`   Org ID: ${credentials.orgId}`)
    console.log(`   Client ID (first 20 chars): ${credentials.clientId?.substring(0, 20)}...`)
    console.log(`   Refresh Token (first 40 chars): ${credentials.refreshToken?.substring(0, 40)}...`)
    console.log(`   Refresh Token (last 40 chars): ...${credentials.refreshToken?.substring(credentials.refreshToken.length - 40)}`)
    console.log(`   Full Refresh Token Length: ${credentials.refreshToken?.length}`)
    console.log("")
    ...
    if (fullTicket.createdBy && typeof fullTicket.createdBy === 'string') {
      console.log("\n🔍 ENRICHMENT: Fetching createdBy agent info")
      console.log(`   Agent ID: ${fullTicket.createdBy}`)
      const createdByAgent = await client.fetchAgentById(fullTicket.createdBy)
      if (createdByAgent) {
        console.log("✅ ENRICHMENT: CreatedBy agent fetched:")
        console.log(JSON.stringify(createdByAgent, null, 2))
        fullTicket.createdBy = createdByAgent as any
      } else {
        console.log("❌ ENRICHMENT: Could not fetch createdBy agent")
      }
    }

    // Fetch modifiedBy agent info if it's just an ID
    if (fullTicket.modifiedBy && typeof fullTicket.modifiedBy === 'string') {
      console.log("\n🔍 ENRICHMENT: Fetching modifiedBy agent info")
      console.log(`   Agent ID: ${fullTicket.modifiedBy}`)
      const modifiedByAgent = await client.fetchAgentById(fullTicket.modifiedBy)
      if (modifiedByAgent) {
        console.log("✅ ENRICHMENT: ModifiedBy agent fetched:")
        console.log(JSON.stringify(modifiedByAgent, null, 2))
        fullTicket.modifiedBy = modifiedByAgent as any
      } else {
        console.log("❌ ENRICHMENT: Could not fetch modifiedBy agent")
      }
    }

    // Fetch account info if we only have accountId
    if (fullTicket.accountId && !fullTicket.account) {
      console.log("\n🔍 ENRICHMENT: Fetching account info")
      console.log(`   Account ID: ${fullTicket.accountId}`)
      const account = await client.fetchAccountById(fullTicket.accountId)
      if (account) {
        console.log("✅ ENRICHMENT: Account fetched:")
        console.log(JSON.stringify(account, null, 2))
        fullTicket.account = account as any
      } else {
        console.log("❌ ENRICHMENT: Could not fetch account")
      }
    }

    // Fetch product info if we only have productId
    if (fullTicket.productId && !fullTicket.product) {
      console.log("\n🔍 ENRICHMENT: Fetching product info")
      console.log(`   Product ID: ${fullTicket.productId}`)
      const product = await client.fetchProductById(fullTicket.productId)
      if (product) {
        console.log("✅ ENRICHMENT: Product fetched:")
        console.log(JSON.stringify(product, null, 2))
        fullTicket.product = product as any
      } else {
        console.log("❌ ENRICHMENT: Could not fetch product")
      }
    }

    // Fetch team info if we only have teamId
    if (fullTicket.teamId && !fullTicket.team) {
      console.log("\n🔍 ENRICHMENT: Fetching team info")
      console.log(`   Team ID: ${fullTicket.teamId}`)
      const team = await client.fetchTeamById(fullTicket.teamId)
      if (team) {
        console.log("✅ ENRICHMENT: Team fetched:")
        console.log(JSON.stringify(team, null, 2))
        fullTicket.team = team as any
      } else {
        console.log("❌ ENRICHMENT: Could not fetch team")
      }
    }

    console.log("\n📊 ENRICHMENT SUMMARY:")
    console.log(`   CreatedBy: ${fullTicket.createdBy?.email || 'not enriched'}`)
    console.log(`   ModifiedBy: ${fullTicket.modifiedBy?.email || 'not enriched'}`)
    console.log(`   Account: ${fullTicket.account?.accountName || 'not enriched'}`)
    console.log(`   Product: ${fullTicket.product?.productName || 'not enriched'}`)
    console.log(`   Team: ${fullTicket.team?.name || 'not enriched'}`)
    console.log("")
...
export async function processAttachmentJob(
  ...
  try {
    ...
    console.log("\n🔑 ATTACHMENT WORKER: Creating Zoho client with stored credentials")
    console.log(`   Connector ID: ${connector.id}`)
    console.log(`   Org ID: ${credentials.orgId}`)
    console.log(`   Client ID (first 20 chars): ${credentials.clientId?.substring(0, 20)}...`)
    console.log(`   Refresh Token (first 40 chars): ${credentials.refreshToken?.substring(0, 40)}...`)
