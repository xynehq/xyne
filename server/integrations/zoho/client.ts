...
export class ZohoDeskClient {
  ...
  async refreshAccessToken(): Promise<string> {
    ...
    const refreshPromise = (async () => {
      try {
        ...
        console.log(
          `   Refresh Token (first 40 chars): ${this.config.refreshToken?.substring(0, 40)}...`,
        )
        console.log(
          `   Refresh Token (last 40 chars): ...${this.config.refreshToken?.substring(this.config.refreshToken.length - 40)}`,
        )
        console.log(
          `   Full Refresh Token Length: ${this.config.refreshToken?.length}`,
        )
        console.log("")
  ...
  async fetchAgentById(agentId: string): Promise<{
    ...
    try {
      ...
      console.log("\n🔍 FETCH AGENT: About to make request")
      console.log(`   Agent ID: ${agentId}`)
      console.log(`   Endpoint: /agents/${agentId}`)
      console.log(`   Full URL: https://${this.config.apiDomain}/api/v1/agents/${agentId}`)
      console.log(`   Current Access Token: ${this.accessToken?.substring(0, 30)}...`)
      console.log("")
