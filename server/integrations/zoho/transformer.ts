...
const logger = getLogger(Subsystem.Integrations).child({ module: "zoho-transformer" })
...
export interface VespaZohoTicketBase {
  ...
  // Custom Fields
  cf_merchant_id_1: string
  cf_product_details: string
  cf_first_response_time: string
  cf_resolution_time_1: string
  cf_on_hold_start_time: string
  cf_on_hold_end_time: string
  cf_escalated_end_date: string
...
export function transformZohoTicketToVespa(
  ...
  // Transform threads
  const vespaThreads = threads.map((thread) => transformThread(thread, ticket.id))
  ...
  const daysToClose = calculateDaysToClose(ticket.createdTime, ticket.closedTime)
  ...
  const vespaDoc: VespaZohoTicketBase = {
    ...
    webUrl: ticket.webUrl || `https://desk.zoho.com/support/tickets/${ticket.id}`,
    sourceType: ticket.source?.type || "",
    channel: ticket.channel || "",
    email: ticket.email || ticket.contact?.email || "",
    ...
    // Custom Fields
    cf_merchant_id_1: ticket.cf?.cf_merchant_id_1 || "",
    cf_product_details: ticket.cf?.cf_product_details || "",
    cf_first_response_time: ticket.cf?.cf_first_response_time || "",
    cf_resolution_time_1: ticket.cf?.cf_resolution_time_1 || "",
    cf_on_hold_start_time: ticket.cf?.cf_on_hold_start_time || "",
    cf_on_hold_end_time: ticket.cf?.cf_on_hold_end_time || "",
    cf_escalated_end_date: ticket.cf?.cf_escalated_end_date || "",
    ...
    closedTime: ticket.closedTime ? new Date(ticket.closedTime).getTime() : null,
    dueDate: ticket.dueDate ? new Date(ticket.dueDate).getTime() : null,
    daysToClose,

    // Derived & NLP Summaries (initially empty, will be filled by NLP processing)
    threadSummary: "",
    commentSummary: "",
 ...
 */
export function hasAttachmentsForProcessing(ticket: VespaZohoTicketBase): boolean {
