...
if (!hasSlackConfig) {
  Logger.warn("Slack environment variables not set. Slack integration will be disabled.");
  Logger.info("To enable Slack integration, set SLACK_BOT_TOKEN and SLACK_APP_TOKEN environment variables.");
}

let app: App | null = null;

// Only initialize Slack app if environment variables are present
if (hasSlackConfig) {
  ...
  if (!process.env.SLACK_BOT_TOKEN!.startsWith("xoxb-")) {
    ...
    try {
      ...
      Logger.error(error, "Failed to initialize Slack app. Slack integration disabled.");
      app = null;
    }
  }
}

// Export the app (which could be null if Slack is not configured)
export default app;

// Export a function to check if Slack is enabled
export const isSlackEnabled = () => app !== null;

// Export a function to safely start the Slack app
export const startSlackApp = async () => {
  if (app) {
    try {
      await app.start();
    ...
    Logger.info("Slack app not initialized - skipping start");
    return false;
  }
};
