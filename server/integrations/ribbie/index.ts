...
class RIBBIECircularDownloader {
  ...
  async clickYearLink(year: number): Promise<void> {
    ...
    for (const selector of yearSelectors) {
      try {
        ...
        yearElement = await this.page.waitForSelector(selector, {
          timeout: 5000,
        })
    ...
    try {
      ...
      await this.page.waitForSelector(`a[id^="${targetYear}"]`, {
        timeout: 5000,
      })
    } catch (error) {
  ...
  async clickAllMonths(year: number): Promise<void> {
    ...
    for (const selector of allMonthsSelectors) {
      try {
        Logger.info(`  🔍 Trying selector: ${selector}`)
        allMonthsElement = await this.page.waitForSelector(selector, {
          timeout: 5000,
        })
  ...
  async getAllCircularsFromTable(): Promise<
    Array<{ href: string; text: string; id: string; department: string }>
  > {
    if (!this.page) throw new Error("Page not initialized")
          ...
          if (cells.length >= 5) {
            ...
            if (linkElement) {
              results.push({
                ...
                text:
                  linkElement.textContent?.trim().substring(0, 80) + "..." ||
                  `Circular ${i - 1}`,
                id:
                  linkElement.getAttribute("href")?.match(/Id=(\d+)/)?.[1] ||
                  `${i - 1}`,
    ...
    const filteredCirculars = allCircularData.filter((circular) => {
      ...
      return (
        dept.includes("payment and settlement system") ||
    ...
    Logger.info(
      `🎯 Filtered to ${filteredCirculars.length} circulars from "${targetDepartment}"`,
    )
    ...
    if (filteredCirculars.length > 3) {
      Logger.info(
        `  ... and ${filteredCirculars.length - 3} more from ${targetDepartment}`,
      )
    }
    ...
    if (filteredCirculars.length === 0) {
      ...
      const uniqueDepartments = [
        ...new Set(allCircularData.map((c) => c.department)),
      ]
  ...
  async navigateToCircular(circular: {
    href: string
    text: string
    id: string
  }): Promise<void> {
  ...
  async downloadPDF(): Promise<{ downloadPath: string }> {
    ...
    for (const selector of pdfSelectors) {
      try {
        Logger.info(`  🔍 Trying selector: ${selector}`)
        pdfElement = await this.page.waitForSelector(selector, {
          timeout: 5000,
        })
  ...
  async createOrGetRBICollection(
    userEmail: string,
    workspaceId: number,
  ): Promise<string> {
    ...
    try {
      ...
      const rbiCollection = collections.find(
        (c) => c.name === "RIBBIE Payment Systems Circulars",
      )
  ...
  async processAndIngestPDF(
    downloadPath: string,
    userEmail: string,
    workspaceId: number,
    circular: { href: string; text: string; id: string; department: string },
  ): Promise<void> {
    ...
    try {
      ...
      Logger.info(
        `👤 Fetched user for ingestion: ${userEmail} and ${users.length} found`,
      )
      if (!users || users.length === 0) {
        throw new Error(`User not found: ${userEmail}`)
      }
      ...
      const collectionId = await this.createOrGetRBICollection(
        userEmail,
        workspaceId,
      )
      ...
      Logger.info(
        `📝 Processing: ${fileName} (${(stats.size / 1024).toFixed(2)} KB)`,
      )
      ...
      Logger.info(
        `✅ Extracted ${processingResult.chunks.length} text chunks and ${processingResult.image_chunks.length} image chunks`,
      )
      ...
      Logger.info(
        `🎉 SUCCESS: RIBBIE PDF fully integrated into Knowledge Base!`,
      )
  ...
  async testCompleteFlow(): Promise<string[]> {
    try {
      ...
      const workspaceId = parseInt(
        process.env.RIBBIE_CONFIG_WORKSPACE_ID || "1",
      )
      ...
      Logger.info(
        `🎯 Starting to process ${years.length} years: ${years.join(", ")}`,
      )
      ...
      for (let yearIndex = 0; yearIndex < years.length; yearIndex++) {
        ...
        try {
          ...
          Logger.info(
            `🎯 Found ${allCirculars.length} circulars for year ${year}`,
          )
          ...
          for (let i = 0; i < allCirculars.length; i++) {
            ...
            try {
              ...
              if (this.isAlreadyDownloaded(circular.id)) {
                Logger.info(
                  `⏭️ Skipping already processed circular ID: ${circular.id}`,
                )
                continue
              }
              ...
              await this.processAndIngestPDF(
                downloadPath,
                userEmail,
                workspaceId,
                circular,
              ...
              Logger.info(
                `✅ [${year}] Successfully processed ${i + 1}/${allCirculars.length}: ${downloadPath}`,
              )
            } catch (circularError) {
              yearErrorCount++
              console.error(
                `❌ [${year}] Failed to process circular ${i + 1}/${allCirculars.length} (${circular.text}):`,
                circularError,
              )
          ...
          Logger.info(`\n✅ Year ${year} COMPLETE!`)
          Logger.info(
            `📊 Year ${year}: ${yearSuccessCount} success, ${yearErrorCount} errors, ${allCirculars.length} total`,
          )
        } catch (yearError) {
          totalFailedYears++
      ...
      Logger.info(
        `   Years processed: ${totalSuccessfulYears}/${years.length} successful`,
      )
      ...
      Logger.info(
        `   Success rate: ${((totalSuccessfulCirculars / totalCirculars) * 100).toFixed(1)}%`,
      )
      Logger.info(
        `📁 All PDFs are now searchable in your "RIBBIE Circulars" Knowledge Base collection!`,
      )
...
if (import.meta.main) {
  if (
    !process.env.RIBBIE_ABSOLUTE_PDF_URL ||
    !process.env.RIBBIE_CONFIG_URL ||
    !process.env.RIBBIE_CONFIG_BASE_URL
  ) {
    throw new Error(
      "Environment variables RIBBIE_ABSOLUTE_PDF_URL, RIBBIE_CONFIG_URL, and RIBBIE_CONFIG_BASE_URL must be set",
    )
  }
