 ...
 */
async function waitForTracerFile(startTime: number): Promise<string | null> {
  console.log(`🔍 Monitoring tracer directory: ${TRACER_DIR}`)
  ...
  while (Date.now() < endTime) {
    try {
      if (!existsSync(TRACER_DIR)) {
        console.log(`⚠️  Tracer directory does not exist: ${TRACER_DIR}`)
        await new Promise((resolve) => setTimeout(resolve, POLLING_INTERVAL_MS))
        continue
      }

      const files = readdirSync(TRACER_DIR)
      const jsonFiles = files.filter((f) => f.endsWith(".json"))

      console.log(`📁 Found ${jsonFiles.length} JSON files in tracer directory`)

      for (const file of jsonFiles) {
        const filePath = join(TRACER_DIR, file)
        try {
          const stats = statSync(filePath)
          const fileCreatedTime = stats.mtime.getTime()

          // Check if file was created after our API call started
          if (fileCreatedTime >= startTime) {
            console.log(
              `✅ Found new tracer file: ${file} (created: ${new Date(fileCreatedTime).toISOString()})`,
            )
            return filePath
          }
        } catch (fileError) {
          console.warn(`⚠️  Error checking file stats for ${file}:`, fileError)
        }
      }
      ...
      console.warn(`⚠️  Error polling tracer directory:`, error)
      await new Promise((resolve) => setTimeout(resolve, POLLING_INTERVAL_MS))
    }
  }

  console.log(`⏰ Timeout reached waiting for tracer file`)
  return null
}
  ...
  try {
    ...
    for (const span of tracerData.spans) {
      ...
      if (
        ...
        console.log(
          `📝 Found final output: ${agenticAnswer.substring(0, 100)}${agenticAnswer.length > 100 ? "..." : ""}`,
        )
      }
    }
    ...
    console.log(
      `✅ Extracted: traceId=${extractedInfo.traceId}, agenticAnswer=${agenticAnswer ? "found" : "not found"}, tools=${toolResults.length}`,
    )
    return extractedInfo
  } catch (error) {
    console.error(`❌ Error reading tracer file ${tracerFilePath}:`, error)
    return {
      error: `Failed to read tracer file: ${error instanceof Error ? error.message : String(error)}`,
      tracerFilePath,
    }
  }
}
...
async function testAPIAndWaitForTracer(
  ...
  // Record start time before making API call
  ...
  console.log(
    `\n🚀 Processing Question ID ${questionId}: "${query.substring(0, 100)}${query.length > 100 ? "..." : ""}"`,
  )
  ...
  try {
    console.log(
      `⏱️  API call start time: ${new Date(apiStartTime).toISOString()}`,
    )
    ...
    const tracerFilePath = await waitForTracerFile(apiStartTime)
    ...
    return {
      answer: null,
      tracerInfo: {
        error: `API call error: ${error instanceof Error ? error.message : String(error)}`,
      },
      success: false,
    }
  }
...
async function validateAuthentication(cookies: string): Promise<boolean> {
  ...
  try {
    const url = `http://localhost:3000/api/v1/message/create?selectedModelConfig=${encodeURIComponent('{"model":"Claude Sonnet 4","reasoning":false,"websearch":false,"deepResearch":false}')}&message=${encodeURIComponent(testQuery)}`
    ...
    console.log(
      `🔐 Authentication validation: ${isValid ? "✅ Valid" : "❌ Invalid"} (Status: ${response.status})`,
    )
    return isValid
  } catch (error) {
...
async function main() {
  ...
  try {
    ...
    console.log(
      `   Processing ${questionsToProcess.length} questions (from index ${startIndex} to ${endIndex - 1})`,
    )
    ...
    for (let i = 0; i < questionsToProcess.length; i++) {
      ...
      console.log("\n" + "=".repeat(100))
      console.log(
        `🚀 Processing Question ${i + 1}/${questionsToProcess.length} (ID: ${questionId})`,
      )
      ...
      if (result.success && result.tracerInfo.agenticAnswer) {
        ...
        console.log(
          `📝 Agentic answer length: ${result.tracerInfo.agenticAnswer.length} chars`,
        )
        console.log(
          `🔧 Tools used: ${result.tracerInfo.toolResults?.length || 0}`,
        )
        console.log(
          `🔍 Tracer info: ${result.tracerInfo.error ? "Error - " + result.tracerInfo.error : "Success"}`,
        )
      } else {
      ...
      console.log(
        `💾 Saved intermediate results to: ${outputPath} (${existingResults.length} existing + ${results.length} new = ${allResults.length} total)`,
      )
    ...
    console.log(
      `📁 Total entries in final file: ${finalResults.length} (${existingResults.length} existing + ${results.length} new)`,
    )
