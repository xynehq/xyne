...
/**
 * Extracts and parses the most complete JSON object or array from a raw string.
 * This function is designed to be resilient to common LLM output issues,
 * like extraneous text, markdown code blocks, and incomplete structures.
 * @param content The raw string from the LLM.
 * @returns The parsed JSON object or array.
 * @throws An error if parsing fails after cleaning.
 */
function extractAndParseJson(content: string): any {
  // 1. Extract from markdown code block if present
  const codeBlockMatch = content.match(/```json\s*([\s\S]*?)\s*```/s)
  let textToParse = codeBlockMatch ? codeBlockMatch[1].trim() : content
  ...
  if (firstBracket !== -1 && (firstBracket < firstBrace || firstBrace === -1)) {
    ...
    throw new Error("No JSON object or array found in the content.")
  }

  let jsonStr = textToParse.substring(startIndex)
  const startChar = jsonStr[0]

  // 3. Balance brackets/braces to find the end of the JSON structure
  let braceCount = 0
  let bracketCount = 0
  let inString = false
  let endIndex = -1
  ...
  for (let i = 0; i < jsonStr.length; i++) {
    ...
    if (!inString) {
      ...
      if (
        (startChar === "{" && braceCount === 0) ||
        (startChar === "[" && bracketCount === 0)
      ) {
        endIndex = i
        break
      }
  ...
  // If no balanced end is found, we proceed with the potentially truncated string.

  // 4. Sanitize the string to remove common JSON errors
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      try {
        ...
        const parsedResult = extractAndParseJson(content)

        // Validate that it's an array; if it's a single object, wrap it in an array.
        if (!Array.isArray(parsedResult)) {
          if (typeof parsedResult === "object" && parsedResult !== null) {
            console.log(
              `ℹ️  LLM returned a single object for batch ${batchNumber}. Wrapping it in an array.`,
            )
            return [parsedResult]
          }
          throw new Error("Parsed result is not an array or a valid object")
        }
