...
class CollectionIndexMigration {
  ...
  async getStatus(): Promise<MigrationStatus> {
    ...
    const status: MigrationStatus = {
      ...
      canRollback: !oldIndexExists && newIndexExists && duplicateCollections.length === 0,
  ...
  async migrate(): Promise<void> {
    ...
    if (!status.oldIndexExists && status.newIndexExists) {
      Logger.info("✅ Migration already completed - new index exists, old index removed")
      return
    }

    if (!status.canMigrate) {
      if (!status.oldIndexExists) {
        Logger.warn("⚠️  Old index doesn't exist - migration may have been partially applied")
      }
  ...
  async rollback(): Promise<void> {
    ...
    if (!status.canRollback) {
      if (status.duplicateCollections.length > 0) {
        Logger.error("❌ Cannot rollback: Found duplicate collection names per workspace:")
        console.table(status.duplicateCollections)
        throw new Error("Rollback blocked: Duplicate collection names exist per workspace")
      }
  ...
  try {
    switch (action) {
      ...
      default:
        ...
        console.log("===============================")
        console.log("Usage:")
        console.log("  bun run migrations/scripts/migrate-collection-index.ts migrate   # Apply migration")
        console.log("  bun run migrations/scripts/migrate-collection-index.ts rollback  # Rollback migration")
        console.log("  bun run migrations/scripts/migrate-collection-index.ts status    # Check status")
        process.exit(1)
    }
  } catch (error) {
    Logger.error("Migration failed:", error)
