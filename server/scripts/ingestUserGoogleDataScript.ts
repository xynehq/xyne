...
import { serviceAccountConnectorId, WHITELISTED_EMAILS ,ingestDrive,ingestEvent,ingestMail} from "./googleConfig"

const Logger = getLogger(Subsystem.Integrations).child({ module: "ServiceAccountIngestionScript" })
...
const runServiceAccountIngestion = async () => {
  try {
    ...
    if (!connector) {
      throw new Error(`Connector with ID ${serviceAccountConnectorId} not found.`)
    }
