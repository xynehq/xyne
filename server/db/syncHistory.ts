...
import { selectSyncHistorySchema, syncHistory, type InsertSyncHistory, type SelectSyncHistory } from "./schema";

export const insertSyncHistory = async (trx: TxnOrClient, history: Omit<InsertSyncHistory, "externalId">): Promise<SelectSyncHistory> => {
    const externalId = createId();  // Generate unique external ID
    const historyWithExternalId = { ...history, externalId }
    const historyArr = await trx.insert(syncHistory).values(historyWithExternalId).returning()
