...
type NewKnowledgeBase = Omit<NewKbItem, 'type' | 'parentId' | 'path' | 'position' | 'totalCount'> & {
  type?: 'knowledge_base';
  parentId?: null;
  path?: string;
  position?: number;
  ...
  const results = await trx
    ...
    .where(
      and(
        ...
        isNull(kbItems.deletedAt),
        or(
          eq(kbItems.ownerId, userId),
          eq(kbItems.isPrivate, false)
        )
      )
    )
    .orderBy(desc(kbItems.updatedAt));
...
export const getKbItemsByParent = async (
  ...
  return await trx
    ...
    .from(kbItems)
    .where(and(
      eq(kbItems.parentId, actualParentId),
      isNull(kbItems.deletedAt)
    ))
...
export const softDeleteKbItem = async (
  ...
  if (item.type === "folder") {
    ...
    const markDescendantsAsDeleted = async (parentId: string): Promise<number> => {
...
export const getAllKbItems = async (
  ...
  const getAllDescendants = async (parentIds: string[]): Promise<KbItem[]> => {
    ...
    const children = await trx
      ...
      .from(kbItems)
      .where(and(
        sql`${kbItems.parentId} IN (${sql.join(parentIds.map(id => sql`${id}`), sql`, `)})`,
        isNull(kbItems.deletedAt)
      ))
      .orderBy(desc(kbItems.type), asc(kbItems.position), asc(kbItems.name));
