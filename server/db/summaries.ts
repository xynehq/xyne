...
export async function insertIndividualSummary(data: {
  ...
  return await db.insert(threadSummaries).values({
...
export async function getCompletedCount(
  ...
  const completed = results.filter(r => r.processingStatus === "completed").length
...
export async function checkAggregatesComplete(
  ...
  const threadAggregate = hasThreads ? await getAggregateSummary(ticketId, "thread") : null
  const commentAggregate = hasComments ? await getAggregateSummary(ticketId, "comment") : null

  const threadComplete = !hasThreads || threadAggregate?.processingStatus === "completed"
  const commentComplete = !hasComments || commentAggregate?.processingStatus === "completed"
...
export async function deleteTicketSummaries(ticketId: string) {
  ...
  await db.delete(ticketAggregatedSummaries).where(eq(ticketAggregatedSummaries.ticketId, ticketId))
}
