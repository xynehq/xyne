...
async function executeKuzuQuery(
  query: string,
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `KuzuDB query failed: ${response.status} ${response.statusText} - ${errorText}`,
      )
    }
...
class KuzuGraphService {
  ...
  async createNode(
    node: Omit<KuzuGraphNode, "created_at" | "updated_at">,
  ): Promise<KuzuGraphNode> {
    ...
    if (!kuzuAvailable) {
      logger.warn(
        `KuzuDB not available: Created node ${nodeWithTimestamps.id} (not persisted)`,
      )
    ...
    try {
      ...
      for (const propName in properties) {
        if (propName.includes("PRIMARY KEY")) {
          ...
          queryParams[actualPropName] = (nodeWithTimestamps as any)[
            actualPropName
          ]
        } else {
      ...
      logger.error(
        error,
        `Failed to create node ${nodeWithTimestamps.id} in KuzuDB`,
      )
      throw error
    }
  }
  ...
  async createEdge(
    edge: Omit<KuzuGraphEdge, "created_at" | "updated_at">,
  ): Promise<KuzuGraphEdge> {
    this.ensureInitialized()
    ...
    if (!kuzuAvailable) {
      logger.warn(
        `KuzuDB not available: Created edge ${edgeWithTimestamps.id} (not persisted)`,
      )
    ...
    try {
      ...
      logger.error(
        error,
        `Failed to create edge ${edgeWithTimestamps.id} in KuzuDB`,
      )
      throw error
    }
  }
  ...
  async getWorkspaceGraph(
    workspaceId: string,
  ): Promise<{ nodes: KuzuGraphNode[]; edges: KuzuGraphEdge[] }> {
    this.ensureInitialized()

    if (!kuzuAvailable) {
      logger.warn(
        `KuzuDB not available: Retrieved empty graph for workspace ${workspaceId}`,
      )
      return { nodes: [], edges: [] }
    }

    try {
      const nodesResult = await executeKuzuQuery(
        "MATCH (n) WHERE n.workspace_id = $workspace_id RETURN n as nodeData",
        { workspace_id: workspaceId },
      )
      const edgesResult = await executeKuzuQuery(
        "MATCH (a)-[r:CONNECTS]->(b) WHERE r.workspace_id = $workspace_id RETURN r as edgeData, a.id as sourceId, b.id as targetId",
        { workspace_id: workspaceId },
      )
      ...
      logger.info(
        `Retrieved ${nodes.length} nodes and ${edges.length} edges for workspace ${workspaceId}`,
      )
      return { nodes, edges }
    } catch (error) {
      logger.error(
        error,
        `Failed to retrieve graph for workspace ${workspaceId}`,
      )
      throw error
    }
  }
  ...
  async deleteNode(nodeId: string): Promise<void> {
    ...
    if (!kuzuAvailable) {
      logger.warn(
        `KuzuDB not available: Deleted node ${nodeId} (not persisted)`,
      )
      return
    }

    try {
      await executeKuzuQuery("MATCH (n) WHERE n.id = $nodeId DELETE n", {
        nodeId,
      })
  ...
  async deleteEdge(edgeId: string): Promise<void> {
    ...
    if (!kuzuAvailable) {
      logger.warn(
        `KuzuDB not available: Deleted edge ${edgeId} (not persisted)`,
      )
      return
    }
    ...
    try {
      await executeKuzuQuery(
        "MATCH ()-[r:CONNECTS]->(b) WHERE r.edge_id = $edgeId DELETE r",
        { edgeId },
      )
  ...
  async clearWorkspaceGraph(workspaceId: string): Promise<void> {
    ...
    if (!kuzuAvailable) {
      logger.warn(
        `KuzuDB not available: Cleared graph for workspace ${workspaceId} (not persisted)`,
      )
      return
    }
  ...
  async getAllEntities(): Promise<{ nodes: any[]; relations: any[] }> {
    ...
    try {
      ...
      logger.info(
        `Total nodes: ${allNodes.length}, Total relations: ${relations.length}`,
      )
  ...
  async getAllDataByPermission(
    permission: string,
  ): Promise<{ nodes: any[]; relations: any[] }> {
    this.ensureInitialized()
    ...
    try {
      ...
      try {
        ...
        logger.info(
          `Node query returned ${nodeResult.rows?.length || 0} results`,
        )
      } catch (queryError) {
      ...
      if (allNodes.length > 0) {
        const nodeWithPermissions = allNodes.find(
          (node: { permissions: string | any[] }) =>
            node.permissions && node.permissions.length > 0,
        )
        if (nodeWithPermissions) {
          logger.info(
            `Sample permissions found: ${JSON.stringify(nodeWithPermissions.permissions)} (searching for: ${permission})`,
          )
        } else {
          logger.warn(
            `No nodes have permissions field or all permissions arrays are empty`,
          )
        }
      }

      logger.info(
        `Found ${allNodes.length} nodes and ${relations.length} relations for permission: ${permission}`,
      )
      ...
      logger.error(
        error,
        `Failed to retrieve data for permission: ${permission}`,
      )
      throw error
    }
  }
