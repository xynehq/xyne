...
import type { VespaAutocomplete, VespaAutocompleteFile, VespaAutocompleteResponse, VespaAutocompleteUser, VespaFile, VespaSearchResponse, VespaSearchResult, VespaUser } from "@/search/types";
import { AutocompleteFileSchema, AutocompleteUserSchema, FileResponseSchema, SearchResultsSchema, UserResponseSchema, type Autocomplete, type AutocompleteResults, type SearchResponse } from "@/shared/types";

// Vespa -> Backend/App -> Client

export const VespaSearchResponseToSearchResult = (resp: VespaSearchResponse): SearchResponse => {
    const { root } = resp
    return {
        ...
        results: root.children.map((child: VespaSearchResult) => {
            ...
            if ((child.fields as VespaFile).sddocname === 'file') {
                ...
                throw new Error(`Unknown schema type: ${(child.fields as any)?.sddocname}`);
            }
        })
    }
}

export const VespaAutocompleteResponseToResult = (resp: VespaAutocompleteResponse): AutocompleteResults => {
    const { root } = resp
    return {
