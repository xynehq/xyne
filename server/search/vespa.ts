...
import { ProductionServerClient } from "./productionServerClient"
...
const vespa =
  prodUrl && apiKey
    ? new ProductionServerClient(prodUrl, apiKey)
    : new VespaClient()
...
export const groupVespaSearch = async (
  ...
  if (hasProdConfig) {
    try {
      const client = new ProductionServerClient(
  ...
  return vespa
    ...
    .catch((err) => {
      if (vespa instanceof ProductionServerClient) {
        Logger.warn(
          err,
          "Prod vespa failed in ifMailDocExist for search, trying fallback",
        )
        return fallbackVespa.ifMailDocExist(email, docId)
      }
      throw err
    })
    .catch((error) => {
  ...
  return vespa
    ...
    .catch((err) => {
      if (vespa instanceof ProductionServerClient) {
        Logger.warn(
          err,
          "Prod vespa failed in ifDocumentsExist for search, trying fallback",
        )
        return fallbackVespa.ifDocumentsExist(docIds)
      }
      throw err
    })
    .catch((error) => {
  ...
  return vespa
    ...
    .catch((err) => {
      if (vespa instanceof ProductionServerClient) {
        Logger.warn(
          err,
          "Prod vespa failed in ifMailDocumentsExist for search, trying fallback",
        )
        return fallbackVespa.ifMailDocumentsExist(mailIds)
      }
      throw err
    })
    .catch((error) => {
  ...
  return vespa
    ...
    .catch((err) => {
      if (vespa instanceof ProductionServerClient) {
        Logger.warn(
          err,
          "Prod vespa failed in ifDocumentsExistInChatContainer for search, trying fallback",
        )
        return fallbackVespa.ifDocumentsExistInChatContainer(docIds)
      }
      throw err
    })
    .catch((error) => {
  ...
  return vespa
    ...
    .catch((err) => {
      if (vespa instanceof ProductionServerClient) {
        Logger.warn(
          err,
          "Prod vespa failed in ifDocumentsExistInSchema for search, trying fallback",
        )
        return fallbackVespa.ifDocumentsExistInSchema(schema, docIds)
      }
      throw err
    })
    .catch((error) => {
