...
export class ProductionServerClient {
  ...
  async ifDocumentsExist(docIds: string[]): Promise<any> {
    return this.makeApiCall<any>("if-documents-exist", { docIds })
  }

  // Proxy method for ifMailDocumentsExist
  async ifMailDocumentsExist(mailIds: string[]): Promise<any> {
    return this.makeApiCall<any>("if-mail-documents-exist", { mailIds })
  }

  // Proxy method for ifDocumentsExistInChatContainer
  async ifDocumentsExistInChatContainer(docIds: string[]): Promise<any> {
    return this.makeApiCall<any>("if-documents-exist-in-chat-container", {
      docIds,
    })
  }

  // Proxy method for ifDocumentsExistInSchema
  async ifDocumentsExistInSchema(
    schema: string,
    docIds: string[],
  ): Promise<any> {
    return this.makeApiCall<any>("if-documents-exist-in-schema", {
      schema,
      docIds,
    })
  }
  ...
  // Proxy method for ifMailDocExist
  async ifMailDocExist(email: string, docId: string): Promise<boolean> {
    return this.makeApiCall<boolean>("if-mail-doc-exist", { email, docId })
  }
