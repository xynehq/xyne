...
export const getPreviousStepOutput = (
  ...
  // Validate inputs
  if (
    !stepData?.prevStepIds ||
    stepData.prevStepIds.length === 0 ||
    !workflowData
  ) {
    return null
  }
  ...
  try {
    ...
    if (!prevStep) {
      console.warn(
        "Previous step not found for template ID:",
        prevStepTemplateId,
      )
      return null
    }
    ...
    if (prevStepTools.length === 0) {
      console.warn(
        "No tool executions found for previous step:",
        prevStep.id || prevStepTemplateId,
      )
      return null
    }
 ...
 */
export const getFileDataFromStepResults = (
