  ...
  const fetchWorkflows = async () => {
    try {
      ...
      if (Array.isArray(workflows.data)) {
        const filteredWorkflows = workflows.data.sort(
          (a, b) =>
  ...
  const fetchTemplates = async () => {
    try {
      ...
      if (Array.isArray(templates.data)) {
        ...
        const placeholders: Template[] = Array.from(
  ...
  const fetchExecutions = async (
    ...
    try {
      ...
      if (response.data && Array.isArray(response.data)) {
        ...
        const convertedExecutions: WorkflowExecution[] = response.data.map(
          (apiExecution) => ({
            id: apiExecution.id,
            ...
            runTime: calculateRunTime(
              apiExecution.createdAt,
              apiExecution.completedAt,
            ),
          }),
        )
  ...
  const handleViewExecution = async (executionId: string) => {
    try {
      ...
      if (executionData) {
        ...
        if (
          executionTemplate &&
          ((executionTemplate.stepExecutions &&
            executionTemplate.stepExecutions.length > 0) ||
            ...
            steps = [
              {
                ...
                createdAt:
                  executionTemplate?.createdAt || new Date().toISOString(),
                updatedAt:
                  executionTemplate?.updatedAt || new Date().toISOString(),
              },
            ]
          }
          ...
          if (steps.length > 0) {
            ...
            console.error(
              "❌ Could not create workflow structure from execution data",
            )
          }
        }
      } else {
                ...
                <div className="space-y-8 flex flex-col flex-1">
                  ...
                  <div className="flex-1 flex flex-col">
                    <div className="flex items-center justify-between mb-6">
                      <h2
                        className="text-gray-900 dark:text-gray-400 uppercase"
                        style={{
                    ...
                    {loading ? (
                      ...
                      (() => {
                        ...
                        let filteredWorkflows = workflows.filter((workflow) =>
                          workflow.name
                            .toLowerCase()
                            .includes(workflowSearchTerm.toLowerCase()),
                        )
                        ...
                        if (workflowFilter === "shared") {
                          filteredWorkflows = filteredWorkflows.filter(
                            (workflow) => workflow.role === "shared",
                          )
                        }
                        if (workflowFilter === "public") {
                          filteredWorkflows = filteredWorkflows.filter(
                            (workflow) => workflow.isPublic,
                          )
                        }
                              ...
                              <div className="grid grid-cols-4 gap-4 w-full">
                                {filteredWorkflows.map((workflow) => (
                                  <WorkflowCard
                                    key={workflow.id}
                                    workflow={workflow}
                                    onViewClick={(templateId) =>
                                      handleViewWorkflow(templateId, false)
                                    } // false = view-only mode
              ...
              {isExecutionMode ? (
                <ExecutedWorkflowRenderer
                  ...
                  onWorkflowUpdate={(updatedWorkflow) => {
                    ...
                    const workflowData = (updatedWorkflow as any).success
                      ? (updatedWorkflow as any).data
                      : updatedWorkflow

                    console.log(
                      "🔄 Setting selectedTemplate with:",
                      workflowData,
                    )
                    setSelectedTemplate({
                      ...workflowData,
                      ...
                      rootWorkflowStepTemplateId:
                        workflowData.rootWorkflowStepTemplateId || "",
                      createdAt:
                ...
                <WorkflowBuilder
                  ...
                  onWorkflowUpdate={(updatedWorkflow) => {
                    ...
                    console.log(
                      "🔄 Parent received workflow update (Builder):",
                      updatedWorkflow,
                    )
