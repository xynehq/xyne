...
const ExistingAgentConfigUI: React.FC<ExistingAgentConfigUIProps> = ({
  ...
  const agentId =
    mode === "preview"
      ...
      : toolData?.val?.agentId ||
        toolData?.value?.agentId ||
  ...
  const agentName =
    mode === "preview"
      ...
      : toolData?.val?.name ||
        toolData?.value?.name ||
        toolData?.config?.name ||
        "Unknown Agent"

  const agentDescription =
    mode === "preview"
      ? previewAgentData?.description || "No description"
      : toolData?.val?.description ||
        toolData?.value?.description ||
        toolData?.config?.description ||
        "No description"
  ...
  const agentModel =
    mode === "preview"
      ...
      : toolData?.val?.model ||
        toolData?.value?.model ||
        toolData?.config?.model ||
        "Unknown Model"
  ...
  dark:text-gray-300"
            ...
            <div
              className="w-full p-3 bg-slate-50 dark:bg-gray-800 
  ...
  text-slate-900 dark:text-gray-300"
          ...
          {/* Info message */}
          <div
            className="p-3 bg-blue-50 dark:bg-blue-900/20 border 
