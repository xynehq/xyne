...
export interface SlackTriggerConfig {
  triggerType: "app_mention" | "direct_message" | "message_in_channel"
  channelId?: string
  title?: string
  description?: string
}
...
export const SlackTriggerConfigUI: React.FC<SlackTriggerConfigUIProps> = ({
  ...
  const validateConfig = (): { valid: boolean; errors: string[] } => {
    ...
    if (config.triggerType === "message_in_channel") {
      if (!config.channelId || config.channelId.trim() === "") {
        validationErrors.push(
          "Channel is required for 'Message in Channel' trigger",
        )
      } else {
        const channelPattern = /^(#[a-z0-9-_]+|C[A-Z0-9]+)$/i
        if (!channelPattern.test(config.channelId.trim())) {
          validationErrors.push(
            "Invalid channel format. Use #channel-name or Slack channel ID",
          )
        }
      }
    }
  ...
  return (
    <div className="fixed top-[80px] right-0 h-[calc(100vh-80px)] bg-white dark:bg-gray-900 border-l border-slate-200 dark:border-gray-700 flex flex-col overflow-hidden z-50 w-[380px] shadow-xl">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
        ...
        {config.triggerType === "message_in_channel" && (
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-gray-300 mb-2">
            ...
            <p className="mt-1 text-sm font-medium text-slate-700 dark:text-gray-300">
              💡 Enter the channel name (e.g., #general) or channel ID
            </p>
          </div>
        )}
