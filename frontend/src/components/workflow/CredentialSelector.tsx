...
import { credentialsAPI } from "./api/ApiHandlers"
...
export function CredentialSelector({
  ...
  const handleCredentialCreated = async (newCredential: any) => {
    try {
  ...
  const handleCredentialUpdated = async (updatedCredential: any) => {
    try {
  ...
  return (
    <>
      ...
      {/* Hidden CredentialModal for creating new credentials */}
      <CredentialModal
      ...
      >
        <button
          ref={(el) => {
            if (el && showCreateModal) {
              // Trigger the modal when showCreateModal becomes true
              el.click()
              setShowCreateModal(false) // Reset the trigger
            }
          }}
          className="hidden"
        />
      ...
      {/* Hidden CredentialModal for editing existing credentials */}
      {editingCredential && (
        <CredentialModal
        ...
        >
          <button
            ref={(el) => {
              if (el && showEditModal) {
                // Trigger the modal when showEditModal becomes true
                el.click()
                setShowEditModal(false) // Reset the trigger
              }
            }}
            className="hidden"
          />
