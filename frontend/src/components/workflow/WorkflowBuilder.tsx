...
const ToolCard: React.FC<{ tool: Tool }> = ({ tool }) => {
  ...
  return (
    <div
      className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-xs font-medium ${getToolColor(tool.type)}`}
    >
...
const StepNode: React.FC<NodeProps> = ({
  ...
  if (step.type === "ai_agent") {
    const isConfigured =
      (step as any).config?.name && (step as any).config?.name.trim() !== ""
        ...
        >
          ...
          <div className="px-4 pb-4">
            <p className="text-gray-600 text-sm leading-relaxed text-left break-words overflow-hidden">
              {(step as any).config?.description ||
  ...
  if (step.type === "email") {
    const isConfigured =
      (step as any).config?.emailAddresses &&
      (step as any).config?.emailAddresses.length > 0
      ...
      <>
        ...
        >
          ...
          <div className="px-4 pb-4">
            <p className="text-gray-600 text-sm leading-relaxed text-left break-words overflow-hidden">
              Send emails to{" "}
              {(step as any).config?.emailAddresses?.join(", ") ||
...
const WorkflowBuilderInternal: React.FC<WorkflowBuilderProps> = ({
  ...
  const executeNode = useCallback(async () => {
    ...
    try {
      ...
      const instantiateResponse = await workflowTemplatesAPI.instantiate(
      ...
      if (instantiateResponse.error || !instantiateResponse.data) {
        throw new Error(
          `Failed to instantiate workflow: ${instantiateResponse.error || "No data returned"}`,
        )
      }
      ...
      try {
        const initialWorkflowResponse = await workflowsAPI.fetchById(
      ...
      const stepCompleteResponse = await workflowsAPI.completeStep(
        workflowInstance.rootStepId,
      )
