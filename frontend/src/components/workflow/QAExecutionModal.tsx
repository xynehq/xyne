...
export function QAExecutionModal({
  ...
  const stepData = workflowData?.stepExecutions?.find(
    (step: any) => step.id === stepId,
  )
  const toolData = workflowData?.toolExecutions?.find(
    (toolExec: any) => toolExec.id === toolId,
  )
  ...
  const questions = useMemo(() => {
    if (stepStatus === "completed" && toolData?.result?.qaResults) {
      ...
      return qaResults.questionsAndAnswers.map((qa: any) => ({
        ...
        answer: qa.answer,
        status:
          qa.answer.startsWith("Error:") ||
          qa.answer.includes("Failed to get answer")
            ? "error"
            : "completed",
      }))
    }
  ...
  const error = useMemo(() => {
    ...
    if (stepStatus === "failed") {
      ...
      if (!toolData?.result?.awaitingUserSelection)
        return "Q&A processing failed"
    }
    if (
      stepStatus &&
      !["completed", "processing", "failed", "active"].includes(stepStatus)
    ) {
  ...
  const currentStep = useMemo(() => {
    ...
    if (stepStatus === "failed") {
      if (
        toolData?.result?.awaitingUserSelection &&
        toolData?.result?.qaMetadata
      )
        return "selectSheet"
  ...
  const executionId =
    workflowData?.id || workflowData?.execution?.id || stepData?.executionId
  ...
  const processQAQuestions = async () => {
    ...
    try {
      ...
      if (!response.ok || !result.success) {
        throw new Error(
          result.error || result.message || "Failed to start Q&A processing",
        )
      }
      ...
      setLocalError(
        `Failed to start Q&A processing: ${error instanceof Error ? error.message : "Unknown error"}`,
      )
    }
  }
          ...
          {currentStep === "selectSheet" && excelMetadata && !error && (
            <div className="space-y-6">
              <div className="text-center">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  Your Excel file "{excelMetadata.filename}" has{" "}
                  {excelMetadata.sheets.length} sheets. Please select the one
              ...
              <div className="space-y-6">
                <div className="text-center">
                  ...
                  <p className="text-sm text-green-600 dark:text-green-400 mb-4">
                    Sheet "{selectedSheetName}" selected
                  </p>
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    Choose the column that contains the questions you want the
                    AI to answer.
                  </p>
                </div>
                ...
                <div className="max-w-md mx-auto">
                  <Select
                    ...
                    onValueChange={handleColumnSelection}
                  >
                    <SelectTrigger className="w-full">
                      <SelectValue placeholder="Choose a column..." />
                    </SelectTrigger>
                    <SelectContent>
                      {excelMetadata.sheets
                        .find(
                          (s: ExcelSheetMeta) => s.name === selectedSheetName,
                        )
          ...
          {currentStep === "processing" && !error && (
            <div className="space-y-6">
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  The backend is processing all questions. This may take a few
                  minutes...
                </p>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div className="text-center">
                ...
                <p className="text-gray-600 dark:text-gray-400 mb-6">
                  {
                    questions.filter(
                      (q: QAQuestion) => q.status === "completed",
                    ).length
                  }{" "}
                  of {questions.length} questions processed successfully.
                </p>
              </div>
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <div
                          className={`text-sm ${qa.status === "error" ? "text-red-600 dark:text-red-400" : "text-gray-600 dark:text-gray-400"}`}
                        >
