...
const getNodeTypeConfig = (type: NodeType | string) => {
  return (
    NODE_TYPE_CONFIG[type as NodeType] || {
      icon: "❓",
      color: "#6b7280",
      label: type || "Unknown",
      size: 16,
...
export const KnowledgeGraph: React.FC = () => {
  ...
  const fetchGraphDataWithPermission = useCallback(
    async (permissionValue?: string) => {
      try {
        setLoading(true)
        setError(null)

        const queryParam = permissionValue
          ? `?permission=${encodeURIComponent(permissionValue)}`
          : ""
            ...
            nodes: [
              {
                ...
                description:
                  "Please refresh the page and log in again to access your knowledge graph.",
                type: "Issue",
                metadata: {
          ...
          nodes: [
            {
              ...
              description:
                "Unable to connect to the backend server. Please ensure you are logged in and the server is running.",
              type: "Issue",
              metadata: {
                created: new Date().toISOString(),
                isError: true,
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response
          .json()
          .catch(() => ({ message: "Failed to delete node" }))
        throw new Error(
          errorData.message || `Failed to delete node (${response.status})`,
        )
      }
  ...
  const nodeTypeCounts = graphData.nodes.reduce(
    (acc, node) => {
      ...
      <div className="w-full h-full flex flex-col">
        ...
        <div className="flex justify-between items-center mt-6 mb-6">
          ...
          <div className="flex items-center gap-4">
            ...
            >
              {loading ? "⏳" : "🔄"}
              <span className="font-mono text-[12px] font-medium">REFRESH</span>
            </Button>
          </div>
        </div>
        ...
        <div className="mb-6 p-4 bg-gradient-to-r from-blue-50 to-green-50 dark:from-blue-900/20 dark:to-green-900/20 rounded-lg border border-blue-200 dark:border-blue-700 shadow-sm">
          ...
          <div className="mt-4 text-xs text-center text-gray-500 dark:text-gray-400">
            💡 Tip: Try using email addresses from the permissions array shown
            in node metadata
          </div>
        </div>
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button
                      variant="outline"
                      size="sm"
                      className="w-full justify-between text-sm h-8 px-1"
                    >
                      {filterType === "all"
                  ...
                  <DropdownMenuContent className="w-72">
                    ...
                    {Object.entries(NODE_TYPE_CONFIG).map(([type, config]) => {
                      ...
                      return (
                        ...
                        >
                          {getNodeTypeConfig(type).icon}{" "}
                          {getNodeTypeConfig(type).label} ({count})
                        </DropdownMenuItem>
                      )
                    })}
                  </DropdownMenuContent>
              ...
              {selectedNode && (
                <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-2 bg-white dark:bg-gray-800">
                  ...
                  <div className="space-y-1 text-sm">
                    ...
                    <div>
                      <strong>Type:</strong>{" "}
                      {getNodeTypeConfig(selectedNode.type).icon}{" "}
                    ...
                    {!selectedNode.id.startsWith("node_") && (
                      <div className="mt-1 p-2 bg-gray-100 dark:bg-gray-700 rounded text-xs text-gray-600 dark:text-gray-400">
                        ℹ️ Auto-generated nodes from your collections and files
                        cannot be deleted directly. Manage them through the
                        Knowledge Base.
                      </div>
                    )}
                  </div>
                ...
                <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-2 bg-white dark:bg-gray-800">
                  ...
                  <div className="space-y-1 text-sm">
                    <div>
                      <strong>From:</strong>{" "}
                      {
                        graphData.nodes.find((n) => n.id === selectedEdge.from)
                          ?.name
                      }
                    </div>
                    <div>
                      <strong>To:</strong>{" "}
                      {
                        graphData.nodes.find((n) => n.id === selectedEdge.to)
                          ?.name
                      }
                    </div>
                    <div>
