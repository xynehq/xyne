...
<Update label="February 2025" description="v0.3.0">
  ...
  * ```Enhancement``` : Made AWS region optional.
  ...
  * ```Bug Fix``` : Add Loader & display error inside HistoryModal UI. 
  * ```Bug Fix``` : Recent style changes created few UI bugs during chat.
  * ```Bug Fix``` : Fix sidebar icon shift, link logo and change tooltip. 
  * ```Bug Fix``` : Added missing context map for attachments.
  * ```Bug Fix``` : Service account gmail worker crashed due to import of server.
  ...
  * ```Bug Fix``` : Fix people and event ranking.      
  * ```Bug Fix``` : Prevent docs cleaning from affecting numeric values.
  * ```Bug Fix``` : Update attachment stats via worker message.
  * ```Bug Fix``` : Fixed vite config to use .env.default in production and docker (change server’s .env.temp).
  * ```Bug Fix``` : Show OAuth error toast only when an error occurs.
  * ```Bug Fix``` : Include threadId in attachments and skip empty/password‐protected PDFs.
