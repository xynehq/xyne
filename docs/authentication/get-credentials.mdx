...
For now in the scope section only add ```https://www.googleapis.com/auth/userinfo.email```,```https://www.googleapis.com/auth/userinfo.profile``` and ```openid```, leave out the rest of it.
