...
### Guides :
<Note>This is the first step.</Note>
<CardGroup>
<Card
...
And that is all 🎉 ! The app will now be available in port ```3000```.  [Go to xyne](http://localhost:3000/)
...
Then proceed to obtain your ```GOOGLE_CLIENT_ID``` and ```GOOGLE_CLIENT_SECRET``` by following this guide:
<Card
...
<Note> Currently the client side has .env variables that refer to port 3000, if you've changed the port number ensure to change the values in the .env as well. </Note>
