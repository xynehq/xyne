...
./init-script.sh
```
This will initialise all the dependencies required by the server to run the application successfully, and will also start the server, which will be listening on port ```3000```.
