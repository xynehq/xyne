...
Inside the ```server``` folder of the ```xyne``` app, create a ```.env``` file using :
```javascript
sudo touch .env
```
...
Create a new ```.env ``` inside the ```frontend``` folder, use the following command : 
```javascript
touch .env
```
...
From the ```server``` folder of the instance, use the following commands: 
```javascript
./init-script.sh
```
