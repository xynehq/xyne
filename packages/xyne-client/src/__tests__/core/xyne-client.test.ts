...
describe("XyneClient", () => {
	...
	function createClient(overrides?: { onTokenExpired?: () => Promise<string> }) {
		return new XyneClient({
			baseUrl: "https://api.test.com/sdk",
			token: "initial-token",
			onTokenExpired: overrides?.onTokenExpired ?? (async () => "refreshed-token"),
		});
	}

	describe("chat", () => {
		it("sends POST with auth header and streams events", async () => {
			const sseEvents: ChatStreamEvent[] = [{ type: "text", content: "hello" }, { type: "done" }];
	...
	describe("search", () => {
		it("sends POST and returns parsed JSON", async () => {
			const body = { results: [{ docId: "d1", title: "T", score: 0.9, content: "text" }] };
	...
	describe("token refresh", () => {
		it("retries on 401 with refreshed token", async () => {
			mockFetch
				.mockResolvedValueOnce(makeJsonResponse({ error: "Token expired" }, 401))
				.mockResolvedValueOnce(
					makeJsonResponse({ results: [{ docId: "d1", title: "T", score: 1, content: "c" }] }),
				);
		...
		it("throws XyneAuthError if refresh also returns 401", async () => {
			mockFetch
				...
				.mockResolvedValueOnce(makeJsonResponse({ error: "still expired" }, 401));
